...
![Іконка Lolly — велика зелено-біла карамелька-льодяник](/info/icon.svg)
...
1. **Креативник задає правила.** Дизайнер створює базовий шаблон у Design, жорстко закодувавши в ньому шрифти й змінні кольорів бренду. Він робить не одну картку — він виконує фундаментальну роботу *один раз*, щоб більше ніколи не локалізувати її вручну.
...
Оскільки схвалення живе в інструменті, а не у файлі (див. [Як Lolly порівнюється з іншими](/info/positioning.html)), масштаб перестає бути проблемою рецензування. Схваліть інструмент для локалізованих карток для соцмереж один раз, а потім згенеруйте з таблиці **10 000 ресурсів 12 мовами** — і жоден із них не потребує нової перевірки від юристів чи від бренд-менеджера, бо шаблон, з якого всі вони походять, уже схвалений.
...
- <!--i:people--> **Людина, у застосунку.** Пакетна сітка `/pro`: вставте або імпортуйте рядки, отримайте по одному готовому ресурсу на рядок, завантажте zip. Без дизайнерських навичок, без тікета, без очікування.
- <!--i:code--> **Розробник, з командного рядка.** CLI запускає *той самий* рушій і *той самий* шлях рендерингу без інтерфейсу, тож інструмент можна прогнати по всіх 10 000 рядках у скрипті або нічному конвеєрі. Виклик `lolly <tool> --field=…` у циклі — це вся інтеграція.
- <!--i:cpu--> **Система або AI-агент, через MCP.** Той самий інструмент, керований програмно, з тією самою якістю й у ще більшому масштабі — бо машині не стане нудно, поки надходять тисячі файлів.
...
- **Ніякої обов'язкової CMS для ресурсів.** Окремі люди завантажують власні творчі файли прямо у свій каталог у застосунку (подання [Каталог](/info/using.html) та Brand Studio) і створюють власні інструменти, зберігаючи сесії [Design](/info/using.html) — без сервера, без адмінконсолі. Для *спільного, керованого* каталогу організація **може** керувати директорією ресурсів як git і гейтити оновлення через рецензування PR — це доступна модель врядування, а не вимога застосунку.
...
| Етап | Ціль | Що |
|---|---|---|
| **Початкові інструменти** | ✅ Готово | QR Code, Quote Card, Email Signature, Code Canvas, Countdown Timer, Color Palette, Brand Lockup, Chart Creator, Filter: Duotone, Meeting Planner — веб-оболонка працює |
