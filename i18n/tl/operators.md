...
> **Ang kasalukuyang estado.** Malakas sa likas na disenyo ang mga security property ng Lolly, at ang mga engine nito para sa cryptography at file-parsing ay dumaraan sa enterprise-grade na infrastructure hardening ng SUSE. Ang mga seal, on-device signing, at encryption sa ibaba ay tunay at maipagtatanggol na ngayon, at humihinog patungo sa independent certification - kaya kung saan hinihiling ng isang kontrata ang sertipikadong katiyakan, i-deploy ang mga ito bilang defence-in-depth habang natatapos ang prosesong iyon.
...
- **Sa iyo ang configuration.** Open source ang engine at mga shell (MPL-2.0). Maglagay ng sarili mong auth, telemetry, o CA sa ibabaw nito; i-host man o hindi; hawak mo ang buong kontrol sa feature at gastos, naka-track sa git, hindi naka-lock sa isang SaaS database.
- **Ang governance ay puwedeng maging data, hindi isang dashboard.** Kapag gusto mo ang kontrol na iyon, pamahalaan ang tool catalog bilang isang Git repository - ang pull-request review ay nagiging brand approval, may kumpletong audit trail at agarang rollback ng bawat template na magagalaw ng workforce mo. Isang opsyon ito, hindi isang obligasyon, at nabibilang ito sa eksaktong isang desk: ang mga creator ay gumagawa nang buo sa loob ng app, sine-save kung ano ang gawa nila bilang isang **session** at ipinapasa ito bilang isang share link, isang backup, o isang live na collaboration - wala sa mga iyon ang nangangailangan ng git. Kapag karapat-dapat nang maging permanenteng starting point ang isa sa mga session na iyon, sinuman ang nagpapatakbo ng deployment ay binubuksan ang link, itinatala ang mga value nito bilang isang **template** sa tool na iyon sa brand pack, at nagko-commit. Mula noon, lalabas ito sa "New from template" chooser ng tool at maaaring i-deep-link bilang `?template=<id>`. Ang Git ang locking step ng admin, ginagamit nang isang beses, at hindi kailanman isang bagay na kailangang galawin ng isang creator. Tingnan ang [Adoption & Governance](/info/adoption-governance.html).
- **Structural ang mga guard-rail.** Naka-hard-code ang mga brand constraint sa mga template, hindi ito nailathala bilang mga guideline na puwedeng balewalain ng mga tao. Hindi lang basta hindi hinihikayat ang maling output - hindi ito puwedeng mabuo.
...
![Strip Hidden Data: dumarating ang file sa canvas at malinaw na sinasabi ng badge na walang na-upload](/t/url-shot?url=%2F%23%2Ftool%2Fstrip-data&width=1200&height=750&dpi=192&waitMs=2000&walker=1&format=svg&dark=1&filename=aud-strip-data)
...
![Prompt to Image - isang blake ng prompt text na naka-typeset sa isang square na larawan, walang anuman sa resulta na wala sa input](/t/url-shot?url=%2F%23%2Ftool%2Fprompt-card%3Ffull&width=1440&height=900&dpi=192&waitMs=2200&walker=1&format=svg&cropSelector=%23tool-canvas&dark=1&filename=ov2-prompt-to-image)
...
## Standalone, o pinamamahalaan ng isang control plane

Dalawang hugis, at pipiliin mo kada deployment. **Ang Standalone ang default at hindi nangangailangan ng server:** nagre-render ang Lolly sa device, gumagawa ang bawat producer sa loob ng app, at ang git-as-data na governance sa itaas ay ganap na opsyonal - isang solong organisasyon ang puwedeng magpatakbo ng repo na ito nang walang kahit anong naka-host. **Kapag gusto mo ng org-wide na kontrol, magdagdag ng isang control plane.** Ang [lolly.work](https://lolly.work) ay isang hiwalay, open-source (MPL-2.0) na serbisyong iho-host mo - o susubukan sa hosted sandbox - na namamahala sa shell nang live: SSO-gated na sign-in, patakaran sa feature-flag / export / watermark, mga overlay ng tool-input, catalog federation, mga approval, at isang hash-chained audit log, na lahat inihahatid sa shell nang walang pagbabago sa code dito. Brand-agnostic ito (config kasama ang isang pack mount), gumagamit ng engine at mga pack ng repo na ito nang hindi binabago, at hindi kailanman nagiging render path: nagre-render pa rin ang Lolly sa device ayon sa disenyo. OSS = kalayaan ng indibidwal; OSS + control plane = kalayaan ng organisasyon.
