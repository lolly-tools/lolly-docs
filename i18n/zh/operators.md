...
- **治理可以是数据，而不是仪表盘。** 当你需要这种管控时，可以把工具目录（catalog）作为 Git 仓库来管理——pull request 审核就成了品牌审批，员工能接触到的每一个模板，你都拥有完整的审计记录并可即时回滚。这是一种可选项，而非义务：只想做东西的团队完全可以在 Design 中编写自己的工具、把自己的文件导入目录（catalogue），全程在应用内完成，从不接触 git。参见[采用与治理](/info/adoption-governance.html)。
