...
- <!--i:trash--> **最干净的切换方式:** **个人资料 → 存储 → 清除我的所有数据**,然后**导入**你要切换到的情境对应的资料包。此后你就纯粹以那份个人资料的身份创作。
- <!--i:layers--> **叠加方式:** 不先清除就直接导入会**合并**——导入的个人资料、会话和图片会叠加在已有内容之上,同名的会被替换,其余保留。适合把某个团队已保存的会话拉进你自己的环境;但如果你需要清晰的角色边界,这就不是你想要的方式。
- <!--i:monitor--> **并行方式:** 由于一切都限定在设备范围内,一个独立的浏览器个人资料、一个独立的用户账户,或另一次安装的 PWA,各自都带有独立的 Lolly 个人资料。你可以同时运行你的个人安装和活动展示终端安装,无需切换。
...
- <!--i:download--> **导出我的数据**会下载一个 `LollyTools-<First>-<Last>-<YYYY-MM-DD>-<n>.zip` 文件——以其所属的个人资料命名,并带有按天区分的序号,避免重复导出时发生冲突(个人资料缺少的部分会在文件名中省略)。它包含你的个人资料、每一个已保存的会话(及其缩略图)、你上传的图片,以及你的偏好设置(主题、版面、本地活动统计)。
- <!--i:upload--> **导入数据……**在另一台安装上读取该文件,你就能从离开的地方原样继续。
...
- <!--i:people--> **个人资料** ——*你*(或你的团队,或你所扮演的角色):姓名、联系方式、头像照片、你保存的作品。个人化、保存在本地设备上、可以作为资料包移动。
- <!--i:palette--> **平台** ——*品牌*本身:颜色、字体,以及每个工具据以渲染的全局设置。共享且一致,不具个人化。
- <!--i:sliders--> **功能** ——*应用程序能做什么*:你可用的完整功能集和工具。
...
- <!--i:box--> **内容配置(Content profile)** —— `profiles.json` 中的一种构建期配置,将一组工具包与某个品牌目录绑定(例如 `suse`、`lolly-start`)。这是运营者在部署时所做的选择;导出时,`profile` 这个 **URL/CLI 参数**选择的也是这个含义下的一种*颜色*变体(即 ICC/CMYK 印刷条件——参见[URL 模式](/info/url-mode.html))。两者说的都是*构建/输出*,而不是*你*。参见[配置](/info/configuration.html)。
- <!--i:seal--> **身份档案(Identity profile)** —— 你可以注册的可选**已验证 Content Credentials 身份**(一种将你的邮箱与已签名导出内容相关联的短期证书)。这是一种签名身份,与个人资料中的姓名/联系方式字段是分开的,不过**使用我的详细信息**决定的是二者是否会被嵌入。参见 [Content Credentials 身份](/info/content-credentials-identity.html)。
