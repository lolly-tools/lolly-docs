...
**超越预设：关键帧、景深和一个镜头。** 转场只在片段进入和离开时为它做动画。若要在片段*内部*摆布一个方框——让它漂移、淡变、模糊，把它抬离纸面再放回去——就添加关键帧：选中片段，按 **+关键帧**（时间轴工具组里的菱形、画布对象栏上的菱形，或 `K`），播放头的位置决定你接下来的编辑写入哪一个姿态。同一套机制为每一段带时间的作品配上一个**镜头**：推进、横摇、变换焦点，并把一张扁平的 SVG 变成可以在其间穿行的层叠。**[动画](/info/animating.html)** 是完整指南。
...
- **展台模式。** 给某个画板设定**时长**，演示就会在那里停留这么久，然后在一条细进度条的伴随下自行前进；`K`（或暂停按钮，只有当某处设了时长时才出现）可以停止和重新开始。在链接里加上 `loop`，演示到末尾会绕回开头，这正是它能当作展示标牌的原因。
...
在实时[协作](/info/collaborate.html)中，历史记录只属于你自己。来自另一台设备的更改绝不会落到你的栈上，因此撤销能收回的永远只有你自己做过的事。
...
批量是*同一*份设计的许多变体。**多重编辑**则是这项工作的另一半：同时打开若干份**不同**的已保存设计，一次修改落到它们全部之上。在**项目**中勾选**二到八个**已保存会话，从选择栏选择**一起编辑**；它们会作为实时卡片并排打开在 `#/multi?s=<slot>,<slot>…`。每张卡片都是该会话的真实渲染，而不是存下来的缩略图，所以你看到的就是它将导出的样子。
