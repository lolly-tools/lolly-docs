...
Uneltele vectoriale (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, lockup-urile de logo, Diagram Builder) pot exporta **DXF** - formatul de interschimb AutoCAD R12 pe care îl citesc plotterele laser, plotterele vinil și software-ul CNC/CAD. Geometria e scrisă ca **căi de contur în milimetri** (curbele sunt aplatizate la o toleranță fină), textul e conturat în căi, iar culoarea ajunge la cel mai apropiat AutoCAD Color Index (care de obicei determină unealta/operația pe un plotter de tăiere). DXF conține doar linii - o regiune fotografică sau filtrată nu are o formă de cale de tăiere și este eliminată (Lolly avertizează), deci folosește SVG/PDF când trebuie să păstrezi conținut raster.
...
**Gândire.** Matematica culorii se întâmplă perceptual, nu pe canale brute. Derivarea paletei, rampele, armoniile și contrastul rulează în **OKLCH/OKLab**, iar o culoare în afara gamei este adusă în interval prin propriul algoritm de mapare a gamei din CSS Color 4 - reducere de crominanță cu verificare a distanței perceptuale - nu prin tăierea canalelor, deci o culoare vie ajunge la cea mai apropiată culoare pe care ai accepta-o de fapt, nu la una aplatizată. Gradientele interpolează într-un spațiu la alegere (OKLab implicit, sau `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, cu o direcție de parcurgere a nuanței pentru cele polare), iar amestecul este **premultiplicat**, deci un fade spre transparent păstrează culoarea corectă în loc să se întunece spre negru pe parcurs. Un singur interpolator servește atât previzualizarea, cât și walker-ele de export - asta a oprit un gradient conic de la a fi amestecat într-un fel pe ecran și altfel în fișierul exportat.
