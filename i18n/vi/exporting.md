...
Các công cụ vector (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, các cụm logo, Diagram Builder) có thể xuất ra **DXF** - định dạng trao đổi AutoCAD R12 mà máy cắt laser, máy cắt decal và phần mềm CNC/CAD đọc được. Hình học được ghi dưới dạng **đường path theo milimét** (các đường cong được làm phẳng theo dung sai tinh), văn bản được vẽ đường viền thành path và màu sắc được ánh xạ về AutoCAD Color Index gần nhất (thường quyết định công cụ/thao tác trên máy cắt). DXF chỉ chứa hình vẽ đường nét - một vùng ảnh chụp hoặc có bộ lọc không có dạng đường cắt nên sẽ bị bỏ qua (Lolly sẽ cảnh báo), vì vậy hãy dùng SVG/PDF khi cần giữ lại nội dung raster.
...
**Tư duy.** Phép toán màu diễn ra theo cảm nhận thị giác thay vì trên các kênh thô. Việc suy ra bảng màu, dải màu, hòa hợp màu và độ tương phản chạy trong **OKLCH/OKLab**, và một màu ngoài dải gam được đưa về trong phạm vi bằng chính thuật toán ánh xạ gam màu của CSS Color 4 - giảm độ bão hòa kèm kiểm tra khoảng cách cảm nhận - thay vì cắt xén từng kênh, nên một màu rực rỡ sẽ rơi vào màu gần nhất mà bạn thực sự chấp nhận được thay vì một màu bị làm phẳng. Gradient nội suy trong một không gian bạn chọn (mặc định là OKLab, hoặc `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, kèm hướng dịch chuyển sắc độ cho các không gian dạng cực), và việc pha trộn là **premultiplied**, nên khi mờ dần sang trong suốt màu vẫn giữ đúng thay vì tối dần về đen giữa chừng. Một bộ nội suy duy nhất phục vụ cả bản xem trước lẫn các walker xuất tệp - điều đã ngăn một gradient hình nón bị pha trộn theo một cách trên màn hình và theo cách khác trong tệp xuất ra.
