...
![Sequence Studio のタイムライン。トランスポート、ルーラー、オーバーレイレーン、クリップと継ぎ目チップが並ぶマグネティックなシーケンス行、そして Always on の帯](/t/url-shot?url=%2F%23%2Ftool%2Fsequence-studio&width=1440&height=900&dpi=192&waitMs=7000&css=.tl-panel%7Bheight%3A252px!important%7D&cropSelector=.tl-panel&format=svg&walker=1&tolerance=0.03&dark=1&filename=seq-studio-timeline)
...
![編集画面の全体。アートボード、ツールレール、タイムラインが揃った状態](/t/url-shot?url=%2F%23%2Ftool%2Fsequence-studio&width=1440&height=900&dpi=192&waitMs=7000&css=.fc-toolbar%7Bopacity%3A1!important%7D&cropSelector=%23tool-stage&format=svg&walker=1&tolerance=0.03&dark=1&filename=seq-studio-stage)
...
**プリセットの先へ。キーフレーム、奥行き、カメラ。** トランジションはクリップの出入りをアニメーションさせます。クリップの*途中*でボックスの状態を作り込むには（ゆっくり動かす、フェードさせる、ぼかす、紙面から浮かせて戻す）、キーフレームを追加します。クリップを選び、**+Keyframe**（タイムラインのツール群にあるひし形、キャンバスのオブジェクトバーのひし形、または `K`）を押すと、再生ヘッドの位置が、次の編集をどの時点の状態として書き込むかを決めます。同じ仕組みにより、時間を持つすべてのコンポジションに、寄り、パン、フォーカス送りができる**カメラ**が備わり、平らな 1 枚の SVG を、間を飛び回れるレイヤーの積み重ねに変えます。詳しくは**[アニメーション](/info/animating.html)**をご覧ください。
...
- **キオスク。** アートボードに **Length** を与えると、その長さだけそこで止まり、細いプログレスバーとともに自動で次へ進みます。`K`（または、いずれかに長さが設定されたときだけ現れる一時停止ボタン）で停止と再開ができます。リンクに `loop` を加えると最後で先頭に戻り、サイネージとして使えます。
...
ライブの[共同編集](/info/collaborate.html)でも、履歴はあなた個人のものです。相手のデバイスから届いた変更が自分の履歴に入ることはないため、取り消しで戻せるのは常に自分が行った操作だけです。
...
Batch は*1 つの*デザインの多数のバリエーションを扱います。**Multi-edit** はその対になる機能で、**別々の**保存済みデザインを同時に開き、1 回の変更をすべてに反映させます。**Projects** で保存済みセッションを **2 〜 8 件**選び、選択バーから **Edit together** を選ぶと、`#/multi?s=<slot>,<slot>…` で横並びのライブカードとして開きます。各カードは保存されたサムネイルではなくそのセッションの実際のレンダリングなので、見えているものがそのまま書き出されます。
