...
> 初めての方へ。アプリのインストール・デプロイについては [Getting Started](/info/getting-started.html) をご覧ください。このページは、開いた後の操作方法について説明します。
...
そのスイッチの隣にあるのが **Neurospicy Mode** です — 作業中に静かにループする、任意の落ち着いたバックグラウンド集中ビートです。ピル型のセレクターからループを選び、隣のボタンで再生・一時停止し、音量を設定できます。**デフォルトではオフ**で、Sound と同様、セッションやデバイスをまたいで記憶されます。Sound をオフにすると、集中ループもミュートされます。
