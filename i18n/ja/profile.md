...
- <!--i:trash--> **最もクリーンな切り替え方法：** **プロフィール → ストレージ → 自分のデータをすべて削除**を実行し、その後、これから入る文脈のバンドルを**インポート**します。これでそのプロフィールとしてのみ作成する状態になります。
- <!--i:layers--> **重ね合わせ：** 先に削除せずにインポートすると**マージ**されます——インポートしたプロフィール、セッション、画像は既存のものの上に重ねられ、同名のものは置き換えられ、それ以外はそのまま残ります。あるチームの保存済みセッションを自分の環境に取り込みたいときには便利ですが、役割をきっちり区切りたい場合には向きません。
- <!--i:monitor--> **並行運用：** すべてがデバイス単位で管理されるため、別のブラウザプロフィール、別のユーザーアカウント、あるいは2つ目としてインストールしたPWAは、それぞれ独立したLollyプロフィールを持ちます。個人用インストールとイベント用キオスクインストールを切り替えることなく同時に実行できます。
...
- <!--i:download--> **自分のデータをエクスポート**を選ぶと、`LollyTools-<First>-<Last>-<YYYY-MM-DD>-<n>.zip`が1つダウンロードされます——ファイル名はそのプロフィールにちなんで付けられ、同日中に複数回エクスポートしても衝突しないよう連番が付きます（プロフィールに該当する情報がない場合、その部分は省略されます）。中身にはプロフィール本体、保存済みの全セッション（サムネイル付き）、アップロードした画像、そして各種設定（テーマ、レイアウト、ローカルの利用統計）が含まれます。
- <!--i:upload--> 移行先のインストールで**データをインポート…**を選ぶとそのファイルが読み込まれ、まさに中断したところから作業を再開できます。
...
- <!--i:people--> **プロフィール** — *あなた自身*（あるいはチーム、あるいは今担っている役割）：氏名、連絡先、ヘッドショット、保存済みの作業内容。個人的で、デバイスにローカルに保存され、バンドルとして持ち運び可能です。
- <!--i:palette--> **プラットフォーム** — *ブランド*：すべてのツールが描画のよりどころとする色、フォント、全体設定。個人のものではなく、共有され一貫しています。
- <!--i:sliders--> **機能** — *アプリでできること*：利用可能な全機能セットとツール一式。
...
- <!--i:box--> **コンテンツプロファイル** — `profiles.json`内のビルド時設定で、一連のツールパックをブランドカタログ（例：`suse`、`lolly-start`）に紐づけるものです。これはデプロイ時に運用担当者が選ぶものであり、また`profile` **URL/CLIパラメータ**がエクスポート時に選択する*色*のバリエーション（ICC/CMYKの印刷条件——[URLモード](/info/url-mode.html)を参照）でもあります。どちらも*ビルド／出力*に関するものであり、*あなた*についてのものではありません。詳しくは[Configuration](/info/configuration.html)をご覧ください。
- <!--i:seal--> **アイデンティティプロファイル** — 任意で登録できる**検証済みのContent Credentialsアイデンティティ**（あなたのメールアドレスと署名済みエクスポートを結びつける短命の証明書）です。これは署名用のアイデンティティであり、個人プロフィールの氏名／連絡先フィールドとは別物ですが、どちらを埋め込むかは**自分の情報を使用**が制御します。詳しくは[Content Credentials Identity](/info/content-credentials-identity.html)をご覧ください。
