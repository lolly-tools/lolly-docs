...
- <!--i:upload--> **Bringen Sie Ihre Dateien mit.** Ziehen Sie ein beliebiges Bild, SVG, Audioclip, Video, Lottie, PDF oder eine PowerPoint-Präsentation auf den Upload-Bereich - oder klicken Sie, um auszuwählen - und es landet sofort in Ihrem Katalog, bereit in jedem Asset-Picker jedes Tools. Ein mehrseitiges PDF oder eine `.pptx` fragt, welche Seiten oder Folien behalten werden sollen - jede wird zu einem SVG-Asset. Laden Sie so viel hoch, wie Sie möchten; es verlässt nie Ihr Gerät.
- <!--i:star--> **Favorisieren Sie, wonach Sie oft greifen.** Versehen Sie ein Asset (oder ein Marken-Farbmuster) mit ★ und es wird oben in jedem Picker angeheftet, sodass Ihr Standard-Logo oder Ihre Standardfarbe einen Klick entfernt ist.
- <!--i:folder--> **Aufräumen.** Ordnen Sie ein Asset einer anderen Gruppe zu, blenden Sie ein gemeinsames Marken-Asset aus, das Sie nicht nutzen (mit **Show hidden**, um es zurückzuholen), oder löschen Sie Ihre eigenen Uploads ganz. Dieselbe Mehrfachauswahl-Geste und die schwebende Aktionsleiste wie bei Projects funktionieren auch hier, sodass all das auf eine ganze Auswahl auf einmal angewendet werden kann.
- <!--i:layers--> **Ein Video vom Hintergrund lösen.** Öffnen Sie die Details eines Videos oder klicken Sie in einem beliebigen Asset-Picker mit der rechten Maustaste auf seine Karte und wählen Sie **Remove background…**, um eine transparente Alternative zu speichern - ein animiertes WebP oder PNG mit echtem Alphakanal. Wählen Sie eine **Method**: Ein **On-device model** schneidet ein Motiv aus einer belebten Szene aus, oder ein **Colour key** stanzt einen gleichmäßig ausgeleuchteten, flachen Hintergrund wie einen Greenscreen oder eine schlichte Wand heraus, mit **Tolerance**, **Softness** und **Spill removal** zum Feinabstimmen der Kante. Der Farbschlüssel braucht keinen Modell-Download und kein Netzwerk, daher wird **Remove background** bei jedem Video angeboten und ist bei sauberem Material oft cleaner. Eine **Resolution**-Einstellung (360, 480, 720 oder 1080p, nie über die Quelle hinaus) tauscht Detail gegen eine kleinere, schnellere Datei. Es läuft als Hintergrundjob auf Ihrem Gerät. Der fertige Ausschnitt landet neben dem Original als eigenes Asset, und das Content Credential des Quellvideos reist als Ingredient mit. (Siehe [Einmal generiert, gleich gerendert](/info/ai-features.html) dafür, warum das Entfernen eines Hintergrunds eine einfache Bearbeitung bleibt.)
