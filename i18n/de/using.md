...
- **Kiosk.** Geben Sie einem Artboard eine **Länge**, hält die Präsentation dort so lange und schaltet dann hinter einem dünnen Fortschrittsbalken selbst weiter; `K` (oder die Pausenschaltfläche, die erst erscheint, sobald etwas eine Länge hat) stoppt und startet das wieder. Hängen Sie `kiosk` an den Link, springt die Präsentation am Ende zurück an den Anfang - und wird damit zur Beschilderung.
