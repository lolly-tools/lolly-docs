...
![Die freie Zeichenfläche von Design - das Artboard mit dem umgebenden Montagebrett](/t/url-shot?url=%2F%23%2Ftool%2Fdesign%3Ftemplate%3D__blank__&width=1440&height=900&dpi=192&waitMs=2000&walker=1&format=svg&localize=1&dark=1&filename=design)
...
![Die Zeitleiste von Sequence Studio: die Transportleiste, das Lineal, eine Overlay-Spur, die magnetische Sequenzzeile mit ihren Clips und Nahtstellen-Chips und der Streifen Immer aktiv](/t/url-shot?url=%2F%23%2Ftool%2Fsequence-studio&width=1440&height=900&dpi=192&waitMs=7000&css=.tl-panel%7Bheight%3A252px!important%7D&cropSelector=.tl-panel&format=svg&walker=1&tolerance=0.03&dark=1&filename=seq-studio-timeline)
...
![Die gesamte Arbeitsfläche: Artboard, Werkzeugleiste und Zeitleiste zusammen](/t/url-shot?url=%2F%23%2Ftool%2Fsequence-studio&width=1440&height=900&dpi=192&waitMs=7000&css=.fc-toolbar%7Bopacity%3A1!important%7D&cropSelector=%23tool-stage&format=svg&walker=1&tolerance=0.03&dark=1&filename=seq-studio-stage)
...
**Über Voreinstellungen hinaus: Keyframes, Tiefe und eine Kamera.** Ein Übergang animiert einen Clip beim Kommen und Gehen. Um eine Box *innerhalb* eines Clips zu setzen - sie treiben lassen, ein- oder ausblenden, weichzeichnen, von der Seite abheben und wieder ablegen -, fügen Sie Keyframes hinzu: Clip auswählen, **+Keyframe** drücken (die Raute in der Werkzeuggruppe der Zeitleiste, die Raute in der Objektleiste der Zeichenfläche oder `K`), und die Position des Playheads entscheidet, welche Pose Ihre nächste Änderung schreibt. Dieselbe Mechanik gibt jeder zeitlich gesetzten Komposition eine **Kamera**, die heranfährt, schwenkt und den Fokus zieht, und macht aus einem flachen SVG einen Stapel von Ebenen, zwischen denen Sie hindurchfliegen können. **[Animieren](/info/animating.html)** ist der vollständige Leitfaden.
...
- **Weiter** geht es mit **Leertaste**, `→`, **Bild ab** oder einem Klick auf den Streifen am rechten Bildschirmrand; zurück mit `←`, **Bild auf** oder dem Streifen am linken Rand. **Pos1** und **Ende** springen zur ersten und zur letzten Folie. Eine kleine Leiste mit Bedienelementen blendet sich ein, sobald Sie den Zeiger bewegen, und verschwindet wieder, wenn Sie aufhören.
- **Übersicht** (`O` oder die Rasterschaltfläche) legt alle Artboards auf einmal in der Anordnung aus, die Sie ihnen auf der Zeichenfläche gegeben haben; klicken Sie eines an, um es zu öffnen.
- **Einblendschritte.** Klicken Sie mit der rechten Maustaste auf eine Box und wählen Sie **Einblenden bei Schritt 1**, **2** oder **3** statt der Voreinstellung **Immer sichtbar**. Diese Box wartet dann, bis Sie auf ihren Schritt weiterschalten, eine Folie kann also stückweise erscheinen; Boxen mit derselben Nummer erscheinen gemeinsam.
- **Referentenansicht** (`S`) öffnet ein zweites Fenster mit der aktuellen Folie, der nächsten, Ihren Notizen zu dieser Folie und einer laufenden Uhr. Blockiert der Browser das Pop-up, weicht sie auf ein Bedienfeld über der Präsentation aus. Notizen werden je Artboard gesetzt und erscheinen nie auf der Folie selbst.
- `B` hält einen schwarzen Bildschirm (jede Taste holt die Folie zurück), `F` kehrt ins Vollbild zurück, und **Escape** schält eine Schicht nach der anderen ab: von der Übersicht zurück zur Präsentation, von der Präsentation zurück zum Editor.
- **Kiosk.** Geben Sie einem Artboard eine **Länge**, hält die Präsentation dort so lange und schaltet dann hinter einem dünnen Fortschrittsbalken selbst weiter; `K` (oder die Pausenschaltfläche, die erst erscheint, sobald etwas eine Länge hat) stoppt und startet das wieder. Hängen Sie `loop` an den Link, springt die Präsentation am Ende zurück an den Anfang - und wird damit zur Beschilderung.
...
In einer laufenden [Zusammenarbeit](/info/collaborate.html) bleibt der Verlauf allein Ihrer. Eine Änderung, die vom anderen Gerät eintrifft, landet nie auf Ihrem Stapel, Rückgängig kann also immer nur etwas zurücknehmen, das Sie selbst getan haben.
...
- **Ob Ihr Name mitgeht.** Ihr Name, Ihre E-Mail-Adresse und Ihre Organisation werden nur dann in die Datei geschrieben, wenn in Ihrem Profil **Meine Daten zum Erstellen verwenden** aktiv ist. Ist es aus, hält die Datei fest, dass sie mit Lolly erstellt wurde und wann - nichts über Sie.
- **Ob lizenzierte Grafiken mitgehen.** Lizenzierte und markengebundene Assets bleiben standardmäßig zurück. Verwendet das Design welche, sagt der Dialog, wie viele, und bietet zwei Schaltflächen - *Ohne sie herunterladen* oder *Einbeziehen und herunterladen* -, denn ein Einbeziehen gibt die tatsächlichen Dateien an alle weiter, die die `.lolly` öffnen.
- **Ob das Tool mitgeht.** **Das Tool einbeziehen** packt die Dateien des Tools neben das Design, sodass es auch auf einem Gerät aufgeht, das dieses Tool nicht hat. Bei einem eigenen Tool ist der Haken gesetzt - bei einem Fork oder einem privaten Marken-Tool, das Ihr Gegenüber vermutlich nicht hat - und nicht gesetzt bei einem, das dem signierten Katalog Byte für Byte entspricht, denn deren Kopie ist bereits dieselbe Datei.

**Eine öffnen.** Ziehen Sie eine `.lolly` auf die App: Die Assets landen in Ihrer Bibliothek, die Sitzung landet in Projekten, und das Tool öffnet sich darauf. Nichts von Ihnen wird überschrieben: Die Sitzung kommt als neuer Speicherplatz an, während ein Asset, das schon auf diesem Gerät liegt, per Prüfsumme erkannt und wiederverwendet statt dupliziert wird. Jeder Teil wird beim Hereinkommen gegen die Prüfsummen der Datei geprüft, eine unterwegs beschädigte Kopie wird also abgewiesen statt halb importiert.
...
- <!--i:upload--> **Bringen Sie Ihre Dateien herein.** Ziehen Sie ein beliebiges Bild, SVG, eine Audiodatei, ein Video, Lottie, PDF oder eine PowerPoint-Präsentation auf den Uploadbereich - oder klicken Sie zum Auswählen - und es landet sofort in Ihrem Katalog, bereit in der Asset-Auswahl jedes Tools. Bei einem mehrseitigen PDF oder einer `.pptx` wird gefragt, welche Seiten oder Folien bleiben sollen - jede wird zu einem SVG-Asset. Nehmen Sie herein, so viel Sie mögen; es verlässt Ihr Gerät nie.
- <!--i:star--> **Machen Sie zu Favoriten, wonach Sie oft greifen.** Vergeben Sie einem Asset (oder einem Marken-Farbmuster) einen ★, und es heftet sich in jeder Auswahl nach oben, sodass Ihr Lieblingslogo oder Ihre Lieblingsfarbe einen Klick entfernt ist.
- <!--i:folder--> **Aufräumen.** Ordnen Sie ein Asset einer anderen Gruppe zu, blenden Sie ein gemeinsam genutztes Marken-Asset aus, das Sie nicht brauchen (mit **Ausgeblendete anzeigen** kommt es zurück), oder löschen Sie Ihre eigenen Uploads ganz. Dieselbe Mehrfachauswahl-Geste und dieselbe schwebende Aktionsleiste wie in Projekten funktionieren auch hier, all das lässt sich also auf eine ganze Auswahl auf einmal anwenden.
...
Batch sind viele Varianten *eines* Designs. Die **Mehrfachbearbeitung** ist die andere Hälfte der Arbeit: mehrere **verschiedene** gespeicherte Designs gleichzeitig geöffnet, sodass eine Änderung auf allen landet. Haken Sie in **Projekten** zwischen **zwei und acht** gespeicherte Sitzungen an und wählen Sie **Gemeinsam bearbeiten** in der Auswahlleiste; sie öffnen sich als lebende Karten nebeneinander unter `#/multi?s=<slot>,<slot>…`. Jede Karte ist ein echtes Rendering dieser Sitzung, keine gespeicherte Miniatur, Sie sehen also, was exportiert wird.
