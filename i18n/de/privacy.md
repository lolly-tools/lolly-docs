...
| Was | Was Ihr Gerät tatsächlich verlässt | Wann (die auslösende Handlung) | Wenn ein Betreiber es blockiert |
|---|---|---|---|
| Tool-Katalog-Sync | Nichts Persönliches - eine Anfrage nach Lollys eigenem öffentlichen Tool- und Asset-Index, an den eigenen Ursprung der App | Beim Start, danach offline zwischengespeichert | Die App läuft mit ihrem zwischengespeicherten Tool-Satz weiter. Sie entdeckt nur keine neuen Tools mehr |
| Ein Tool, das Live-Daten braucht | Was auch immer dieses konkrete Tool anfragt, an den in seiner eigenen Beschreibung genannten Host. Derzeit ist das nur die Städtesuche im Tool Meeting Planner, die `geocoding-api.open-meteo.com` bittet, einen Stadtnamen in Koordinaten und eine Zeitzone umzuwandeln - kein Konto, kein Schlüssel und keine Kennung über die Anfrage selbst hinaus. Das steht genau dort, wo Sie tippen, und jede Antwort wird auf Ihrem Gerät gespeichert, sodass eine Stadt nur einmal nachgeschlagen wird | Nur während der Nutzung dieses Tools, und nur sobald Sie einen Ort eingeben | Nur diese eine Suche schlägt fehl. Sie können Koordinaten weiterhin von Hand eingeben, und nichts anderes ist betroffen |
| Google Fonts | Der Name der gewählten Schriftfamilie und Ihre IP-Adresse, an Googles Schriftserver (`fonts.googleapis.com` für das Stylesheet, `fonts.gstatic.com` für die Schriftdatei) | Nur wenn Sie im Brand-Editor eine Google-Schriftart hinzufügen, **und erst, nachdem Sie in einem Dialog zugestimmt haben, der genau das sagt** - ein einmaliger Abruf pro Familie, danach liegt sie auf Ihrem Gerät und wird offline verwendet | Die Google-Fonts-Auswahl bleibt dann deaktiviert. Laden Sie stattdessen eine Schriftdatei hoch |
| An Google Drive senden | Die eine Datei, die Sie zum Senden ausgewählt haben, an Googles Drive-API (`www.googleapis.com`), nach einer Google-Anmeldung, die Sie in Googles eigenem Popup-Fenster abschließen. Lollys Zugriff ist auf von ihm erstellte Dateien beschränkt (der Berechtigungsbereich `drive.file` - es kann niemals den Rest Ihres Drive lesen), und das Anmelde-Token wird nur im Speicher für die Sitzung gehalten, nie gespeichert | Nur wenn Sie bei einem EMF-Export auf „An Google Drive senden“ klicken, und nur bei Builds, bei denen der Betreiber eine Google-Client-ID konfiguriert hat - ohne diese existiert die Schaltfläche nicht | Die Schaltfläche erscheint nie. Laden Sie die Datei herunter und selbst zu Drive hoch |
| An Dropbox senden | Die eine Datei, die Sie zum Senden ausgewählt haben, an Dropboxs API (`api.dropboxapi.com` für Anmeldung und Metadaten, `content.dropboxapi.com` für die Datei selbst), nach einer Dropbox-Anmeldung, die Sie in Dropboxs eigenem Fenster abschließen. Lollys Zugriff ist auf den App-Ordner beschränkt (es sieht ausschließlich `Apps/` und den eigenen Ordner darin - nie den Rest Ihrer Dropbox), der Ihnen gezeigte „Öffnen“-Link ist ein kurzlebiger privater Link (es wird keine öffentliche Freigabe erstellt), und ein Refresh-Token wird nur gespeichert, wenn Sie „Verbunden bleiben“ ankreuzen | Nur wenn Sie bei einer Datei auf „An Dropbox senden“ klicken, und nur bei Builds, bei denen der Betreiber eine Dropbox-Client-ID konfiguriert hat - ohne diese existiert die Schaltfläche nicht | Die Schaltfläche erscheint nie. Laden Sie die Datei herunter und selbst zu Dropbox hoch |
| An OneDrive senden | Die eine Datei, die Sie zum Senden ausgewählt haben, an Microsofts Identitäts- und Graph-Dienste (`login.microsoftonline.com` für die Anmeldung, `graph.microsoft.com` für den Upload; eine große Datei wird in Teilen an eine Microsoft-eigene Upload-Adresse unter `api.onedrive.com`, `*.up.1drv.com` oder `*.sharepoint.com` hochgeladen), nach einer Microsoft-Anmeldung, die Sie in Microsofts eigenem Fenster abschließen. Lollys Zugriff ist auf den eigenen Ordner unter `Apps/` beschränkt (es kann niemals den Rest Ihres OneDrive lesen), dazu Ihr Anzeigename für die Kontobezeichnung, und ein Refresh-Token wird nur gespeichert, wenn Sie „Verbunden bleiben“ ankreuzen | Nur wenn Sie bei einer Datei auf „An OneDrive senden“ klicken, und nur bei Builds, bei denen der Betreiber eine Microsoft-Client-ID konfiguriert hat - ohne diese existiert die Schaltfläche nicht | Die Schaltfläche erscheint nie. Laden Sie die Datei herunter und selbst zu OneDrive hoch |
| ICC-Druckprofile | Nichts Persönliches - eine Anfrage nach einem Standard-Druckbedingungsprofil, an das öffentliche Register des ICC (`registry.color.org`, `www.color.org`) | Nur wenn Sie im Druckprofil-Manager eine ICC-Voreinstellung anklicken - ein einmaliger Abruf pro Profil, danach liegt es auf Ihrem Gerät | ICC-Voreinstellungen schlagen fehl. Stellen Sie stattdessen Ihr eigenes `.icc`-Profil bereit |
| Internetradio | Nichts Persönliches - eine Playlist-Anfrage und ein Audiostream, an den Sender (`api.somafm.com` und den von ihm genannten Icecast-Server, `*.somafm.com`) | Nur während Sie das optionale eingebaute Radio im Sound-Player abspielen | Das Radio fällt aus. Jede andere Tonfunktion funktioniert weiterhin |
| Eine URL, die Sie ein Tool erfassen lassen | Eine Anfrage an genau die Webadresse, die Sie eingeben, vom URL-Screenshot-Tool. Ganz gleich, welche Adresse das ist. Dieser Host steht nicht in der Richtlinie unten, weil Sie ihn im Moment der Nutzung selbst wählen | Nur wenn Sie in diesem Tool eine URL eingeben und die Erfassung starten | Ein Betreiber kann dies nicht per Host auf eine Zulassungsliste setzen. Um es zu entfernen, entfernen Sie das Tool |
| SEAL-Signaturprüfung | **Nichts.** Die Web-App hat überhaupt keinen DNS-Resolver - siehe unten | Nie | Nichts zu blockieren |
| Deep-Scan-Erkennungsmodelle | Nichts Persönliches - ein einmaliger Modell-Download vom selben Ursprung (kein Drittanbieter) | Nur wenn Sie sich für den Deep Scan von Verify entscheiden | Deep Scan ist nicht verfügbar. Die Standardprüfung funktioniert weiterhin |
| Remote-Instanz | Was auch immer die von Ihnen genannte Instanz zurückgibt, über denselben oben beschriebenen Katalog-Sync - plus ein Versions-Tag bei Anfragen an sie (Shell-Art und Engine-Version, dieselbe Information, die auch ein User-Agent trägt), damit ihr Betreiber sehen kann, welche Lolly-Versionen im Einsatz sind. Bei einer verwalteten Instanz trägt dieses Tag, solange Sie angemeldet sind, zusätzlich eine geräteweise Installations-ID, damit die Geräteliste des Betreibers diese Installation unterscheiden kann. Sie reist nur mit Anfragen mit, die Ihre eigene Nutzung ohnehin auslöst - es gibt keinen Timer und nichts meldet sich von selbst - und beim Verlassen der Instanz wird die ID gelöscht, sodass ein später erneut verbundenes Gerät eine neue erhält. Sie wählen den Host im Moment der Nutzung selbst, daher steht er nicht in der Richtlinie unten | Nur wenn Sie die Shell ausdrücklich auf eine andere Lolly-Bereitstellung verweisen | Der Instanzwechsel schlägt fehl. Ihre lokale Instanz ist nicht betroffen |
