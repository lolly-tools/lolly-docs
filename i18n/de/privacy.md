...
> **In einfachen Worten.** Die Dokumente, Bilder, Videos und Dateien, die Sie in Lolly erstellen, bleiben
> auf Ihrem Gerät. Es gibt keine Konten für die normale Nutzung, keine Cookies von der App
> selbst und keine Analyse- oder Tracking-Tools irgendwo im Quellcode - nicht "wir nutzen
> die Daten nicht", sondern schlicht nicht vorhanden im Quellcode. Es gibt eine kurze, vollständige Liste von
> Ausnahmen, bei denen die Software überhaupt mit einem Netzwerk spricht, und jede davon
> ist unten im Detail beschrieben: was verlässt das Gerät, an wen und wann. Die einzige
> Ausnahme, die etwas Persönliches betrifft, ist eine Anmeldung, die Sie ausdrücklich
> selbst starten müssen. Steht es nicht in diesem Dokument, geschieht es nicht.
...
|---|---|---|---|
| Tool-Katalog-Abgleich | Nichts Persönliches - eine Anfrage nach Lollys eigenem öffentlichen Tool- und Asset-Index, an den eigenen Ursprung der App | Beim Start, danach offline zwischengespeichert | Die App läuft mit ihrem zwischengespeicherten Tool-Satz. Sie kann nur keine neuen Tools mehr entdecken |
| Ein Tool, das Live-Daten benötigt | Was auch immer dieses konkrete Tool anfordert, an den in seiner eigenen Beschreibung genannten Host. Heute ist das nur die Stadtsuche im Meeting-Planner-Tool, die `geocoding-api.open-meteo.com` bittet, einen Stadtnamen in Koordinaten und eine Zeitzone umzuwandeln - kein Konto, kein Schlüssel und keine Kennung über die Anfrage selbst hinaus. Die Eingabe weist genau dort, wo Sie tippen, darauf hin, und jede Antwort wird auf Ihrem Gerät gespeichert, sodass eine Stadt nur einmal nachgeschlagen wird | Nur während der Nutzung dieses Tools, und nur sobald Sie einen Ort eingeben | Nur diese eine Suche schlägt fehl. Sie können weiterhin Koordinaten von Hand eingeben, und nichts anderes ist betroffen |
| Google Fonts | Der gewählte Schriftfamilienname und Ihre IP-Adresse, an Googles Font-Server (`fonts.googleapis.com` für das Stylesheet, `fonts.gstatic.com` für die Schriftdatei) | Nur wenn Sie eine Google-Schrift im Brand-Editor hinzufügen, **und nur nachdem Sie dem in einem Dialog zustimmen, der genau das besagt** - ein einmaliger Abruf pro Familie, danach lebt sie auf Ihrem Gerät und wird offline genutzt | Der Google-Fonts-Picker schlägt geschlossen fehl. Laden Sie stattdessen eine Schriftdatei hoch |
| An Google Drive senden | Die eine Datei, die Sie zum Senden ausgewählt haben, an Googles Drive-API (`www.googleapis.com`), nach einer Google-Anmeldung, die Sie in Googles eigenem Popup-Fenster abschließen. Lollys Zugriff ist auf Dateien beschränkt, die es selbst erstellt hat (der `drive.file`-Bereich - es kann nie den Rest Ihres Drives lesen), und das Anmelde-Token wird nur für die Dauer der Sitzung im Speicher gehalten, nie gespeichert | Nur wenn Sie "An Google Drive senden" bei einem EMF-Export drücken, und nur bei Builds, bei denen der Betreiber eine Google-Client-ID konfiguriert hat - ohne eine solche existiert die Schaltfläche nicht | Die Schaltfläche erscheint nie. Laden Sie die Datei herunter und selbst zu Drive hoch |
| An Dropbox senden | Die eine Datei, die Sie zum Senden ausgewählt haben, an Dropboxs API (`api.dropboxapi.com` für Anmeldung und Metadaten, `content.dropboxapi.com` für die Datei selbst), nach einer Dropbox-Anmeldung, die Sie in Dropboxs eigenem Fenster abschließen. Lollys Zugriff ist auf den App-Ordner beschränkt (es sieht nur `Apps/` und seinen eigenen Ordner darin - nie den Rest Ihrer Dropbox), der "Öffnen"-Link, den es Ihnen zeigt, ist ein kurzlebiger privater Link (es wird keine öffentliche Freigabe erstellt), und ein Refresh-Token wird nur gespeichert, wenn Sie "verbunden bleiben" ankreuzen | Nur wenn Sie "An Dropbox senden" bei einer Datei drücken, und nur bei Builds, bei denen der Betreiber eine Dropbox-Client-ID konfiguriert hat - ohne eine solche existiert die Schaltfläche nicht | Die Schaltfläche erscheint nie. Laden Sie die Datei herunter und selbst zu Dropbox hoch |
| An OneDrive senden | Die eine Datei, die Sie zum Senden ausgewählt haben, an Microsofts Identitäts- und Graph-Dienste (`login.microsoftonline.com` für die Anmeldung, `graph.microsoft.com` für den Upload; eine große Datei wird in Blöcken an eine von Microsoft betriebene Upload-Adresse unter `api.onedrive.com`, `*.up.1drv.com` oder `*.sharepoint.com` hochgeladen), nach einer Microsoft-Anmeldung, die Sie in Microsofts eigenem Fenster abschließen. Lollys Zugriff ist auf seinen eigenen Ordner unter `Apps/` beschränkt (es kann nie den Rest Ihres OneDrive lesen) plus Ihren Anzeigenamen für die Konto-Beschriftung, und ein Refresh-Token wird nur gespeichert, wenn Sie "verbunden bleiben" ankreuzen | Nur wenn Sie "An OneDrive senden" bei einer Datei drücken, und nur bei Builds, bei denen der Betreiber eine Microsoft-Client-ID konfiguriert hat - ohne eine solche existiert die Schaltfläche nicht | Die Schaltfläche erscheint nie. Laden Sie die Datei herunter und selbst zu OneDrive hoch |
| ICC-Druckprofile | Nichts Persönliches - eine Anfrage nach einem Standard-Druckbedingungsprofil, an das öffentliche Register der ICC (`registry.color.org`, `www.color.org`) | Nur wenn Sie eine ICC-Voreinstellung im Druckprofil-Manager anklicken - ein einmaliger Abruf pro Profil, danach lebt es auf Ihrem Gerät | ICC-Voreinstellungen schlagen fehl. Stellen Sie stattdessen Ihr eigenes `.icc`-Profil bereit |
| Internetradio | Nichts Persönliches - eine Playlist-Anfrage und ein Audiostream, an den Sender (`api.somafm.com` und den von ihm benannten Icecast-Server, `*.somafm.com`) | Nur während Sie das optionale eingebaute Radio im Sound-Player abspielen | Das Radio schlägt fehl. Jede andere Sound-Funktion funktioniert weiterhin |
| Eine URL, die Sie ein Tool erfassen lassen | Eine Anfrage an genau die Webadresse, die Sie eingeben, vom URL-Screenshot-Tool. Was auch immer diese Adresse ist. Dieser Host steht nicht in der untenstehenden Richtlinie, weil Sie ihn im Moment der Nutzung wählen | Nur wenn Sie eine URL in diesem Tool eingeben und die Erfassung starten | Ein Betreiber kann dies nicht nach Host auf die Positivliste setzen. Um es zu entfernen, entfernen Sie das Tool |
| SEAL-Signaturprüfung | **Nichts.** Die Web-App hat überhaupt keinen DNS-Resolver - siehe unten | Nie | Nichts zu blockieren |
| Deep-Scan-Erkennungsmodelle | Nichts Persönliches - ein einmaliger Modell-Download vom gleichen Ursprung (nicht von Dritten) | Nur wenn Sie sich für den Deep-Scan von Verify entscheiden | Deep-Scan ist nicht verfügbar. Die Standardverifizierung funktioniert weiterhin |
| Remote-Instanz | Was auch immer die von Ihnen benannte Instanz zurückliefert, über denselben oben beschriebenen Katalogabgleich. Sie wählen den Host im Moment der Nutzung, daher steht er nicht in der untenstehenden Richtlinie | Nur wenn Sie die Shell ausdrücklich auf eine andere Lolly-Bereitstellung verweisen | Der Instanzwechsel schlägt fehl. Ihre lokale Instanz ist nicht betroffen |

Jeder feste Host in dieser Tabelle ist auch die vollständige Positivliste in der Content-Security-Policy
der App, die der Browser durchsetzt. Die Liste ist also nicht nur eine
Beschreibung dessen, was der Code heute tut, sondern die Grenze, an die der Browser die
App bindet: eine künftige Änderung, die versuchte, einen anderen Host zu kontaktieren, würde blockiert,
nicht stillschweigend erlaubt. Zwei Zeilen haben keinen festen Host, weil Sie die
Adresse im Moment der Nutzung wählen: eine URL, die Sie ein Tool erfassen lassen, und eine Remote-
Instanz, auf die Sie die Shell verweisen. Keine von beiden steht in der Richtlinie, und jede tritt nur
auf, wenn Sie eine Adresse eingeben und danach handeln. Eine Bereitstellung, die keine der
optionalen möchte (eine Enterprise-Instanz mit eigenen Schriften etwa), entfernt diese
Hosts aus ihrer Richtlinie, und die Funktionen scheitern geschlossen, statt nach außen zu greifen.
...
- <!--i:usercheck--> **Keine Konten, keine Cookies, kein Zustand.** Der Endpunkt ist anonym, und nichts
  auf Ihrem Gerät wird gelesen. Ihre Dokumente, Sitzungen und Uploads verlassen nie Ihren
  Browser - sie können in diesen Links überhaupt nicht auftauchen.
- <!--i:document--> **Aber die URL selbst wird protokolliert.** Die Query-String einer URL ist Teil der Anfragezeile,
  daher landet sie in den gewöhnlichen Zugriffsprotokollen der Hosting-Plattform, genauso wie
  jeder angefragte Pfad. Wenn die Eingaben eines Links den Namen oder die E-Mail-Adresse einer Person enthalten -
  ...
  hier ausgeschaltet statt eingeschaltet ist.
- <!--i:globe--> **Die Eingaben sind ohnehin von Natur aus öffentlich** - es ist, was auch immer der Verfasser des Links
  in die URL getippt hat, lesbar für jeden, den der Link erreicht. Legen Sie keine Geheimnisse in
  einen geteilten Link. Lolly bietet Link-Verschlüsselung für sensible Inhalte an.
