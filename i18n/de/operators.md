...
> **Der aktuelle Stand.** Lollys Sicherheitseigenschaften sind von Grund auf robust, und seine Kryptografie- und Dateiparser-Engines durchlaufen SUSEs Infrastrukturhärtung auf Enterprise-Niveau. Die Siegel, die Signatur auf dem Gerät und die Verschlüsselung unten sind schon jetzt real und belastbar und reifen in Richtung unabhängiger Zertifizierung - wo ein Vertrag zertifizierte Absicherung verlangt, setzen Sie sie also als zusätzliche Verteidigungsebene ein, während dieser Prozess läuft.
...
- **Die Konfiguration gehört Ihnen.** Engine und Shells sind Open Source (MPL-2.0). Legen Sie Ihre eigene Authentifizierung, Telemetrie oder CA darüber; hosten Sie es oder lassen Sie es; Sie behalten volle Kontrolle über Funktionen und Kosten, git-verfolgt, nicht in einer SaaS-Datenbank eingesperrt.
- **Governance kann Daten sein, kein Dashboard.** Wenn Sie diese Kontrolle wollen, verwalten Sie den Tool-Katalog als Git-Repository - Pull-Request-Review wird zur Markenfreigabe, mit vollständigem Audit-Trail und sofortigem Rollback jeder Vorlage, die Ihre Belegschaft anfassen kann. Das ist eine Option, keine Pflicht, und sie gehört genau einem Schreibtisch: Ersteller arbeiten vollständig in der App, speichern, was sie machen, als **Sitzung** und geben es als Freigabelink, Backup oder Live-Zusammenarbeit weiter - nichts davon braucht Git. Wenn eine dieser Sitzungen es verdient, ein dauerhafter Ausgangspunkt zu werden, öffnet, wer auch immer die Bereitstellung betreibt, den Link, hält dessen Werte als **Vorlage** für dieses Tool im Markenpaket fest und committet. Von da an erscheint sie in der „Neu aus Vorlage“-Auswahl des Tools und ist als `?template=<id>` per Deep Link erreichbar. Git ist der verriegelnde Schritt der Administration, einmal genutzt, und nie etwas, das eine Erstellerin oder ein Ersteller anfassen muss. Siehe [Einführung & Governance](/info/adoption-governance.html).
- **Leitplanken sind strukturell.** Markenvorgaben sind fest in Vorlagen codiert, nicht als Richtlinien veröffentlicht, die man ignorieren kann. Das falsche Ergebnis wird nicht bloß entmutigt - es ist überhaupt nicht darstellbar.
...
![Strip Hidden Data: Die Datei kommt auf der Leinwand an, und das Badge stellt unmissverständlich klar, dass nichts hochgeladen wird](/t/url-shot?url=%2F%23%2Ftool%2Fstrip-data&width=1200&height=750&dpi=192&waitMs=2000&walker=1&format=svg&dark=1&filename=aud-strip-data)
...
![Prompt to Image - ein Block Prompt-Text, gesetzt in ein quadratisches Bild, wobei im Ergebnis nichts steht, was nicht schon in der Eingabe stand](/t/url-shot?url=%2F%23%2Ftool%2Fprompt-card%3Ffull&width=1440&height=900&dpi=192&waitMs=2200&walker=1&format=svg&cropSelector=%23tool-canvas&dark=1&filename=ov2-prompt-to-image)
...
## Eigenständig oder von einer Steuerungsebene verwaltet

Zwei Formen, und Sie wählen pro Bereitstellung. **Eigenständig ist die Standardeinstellung und braucht keinen Server:** Lolly rendert auf dem Gerät, jede Erstellerin und jeder Ersteller arbeitet in der App, und die oben beschriebene Git-als-Daten-Governance ist vollkommen optional - eine einzelne Organisation kann dieses Repository betreiben, ohne überhaupt etwas zu hosten. **Wenn Sie organisationsweite Kontrolle wollen, fügen Sie eine Steuerungsebene hinzu.** [lolly.work](https://lolly.work) ist ein separater, quelloffener (MPL-2.0) Dienst, den Sie selbst hosten - oder in der gehosteten Sandbox ausprobieren -, der die Shell live steuert: SSO-geschützte Anmeldung, Feature-Flag-/Export-/Wasserzeichen-Richtlinien, Tool-Eingabe-Overlays, Katalogföderation, Freigaben und ein hash-verkettetes Audit-Log, alles an die Shell ausgeliefert, ohne hier eine einzige Codezeile zu ändern. Er ist markenunabhängig (Konfiguration plus ein Paket-Mount), nutzt die Engine und die Pakete dieses Repositorys unverändert und wird niemals zum Renderpfad: Lolly rendert weiterhin von Grund auf auf dem Gerät. OSS = individuelle Freiheit; OSS + Steuerungsebene = organisationale Freiheit.
