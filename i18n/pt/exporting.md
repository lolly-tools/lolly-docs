...
Ferramentas vetoriais (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, os lockups de logotipo, Diagram Builder) podem exportar **DXF** - o formato de intercâmbio AutoCAD R12 que cortadoras a laser, plotters de vinil e software CNC/CAD leem. A geometria é escrita como **caminhos de contorno em milímetros** (curvas achatadas com uma tolerância fina), o texto é convertido em contornos de caminho e a cor é mapeada para o índice de cor AutoCAD mais próximo (que costuma determinar a ferramenta/operação em uma cortadora). DXF é apenas arte de linha - uma região fotográfica ou filtrada não tem forma de caminho de corte e é descartada (o Lolly avisa), então use SVG/PDF quando precisar manter conteúdo raster.
...
**Pensamento.** A matemática de cor acontece de forma perceptual, não em canais brutos. Derivação de paleta, rampas, harmonias e contraste rodam em **OKLCH/OKLab**, e uma cor fora do gamute é trazida para dentro do intervalo pelo próprio algoritmo de mapeamento de gamute do CSS Color 4 - redução de croma com uma verificação de distância perceptual - em vez de recortar canais, então uma cor vívida cai na cor mais próxima que você de fato aceitaria em vez de uma versão achatada. Gradientes interpolam em um espaço que você escolhe (OKLab por padrão, ou `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, com uma direção de percurso de matiz para os polares), e a mistura é **pré-multiplicada**, então um desvanecimento para transparente mantém a cor correta em vez de escurecer em direção ao preto pelo caminho. Um único interpolador serve tanto a pré-visualização quanto os walkers de exportação - o que foi o que impediu um gradiente cônico de ser misturado de um jeito na tela e de outro no arquivo exportado.
