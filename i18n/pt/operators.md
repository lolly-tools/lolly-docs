...
> **O estado atual.** As propriedades de segurança do Lolly são fortes por design, e seus motores de criptografia e análise de arquivos estão passando pelo endurecimento de infraestrutura de nível empresarial da SUSE. Os selos, a assinatura no dispositivo e a criptografia abaixo são reais e defensáveis agora, e estão amadurecendo rumo à certificação independente - então, onde um contrato exige garantia certificada, use-os como defesa em profundidade enquanto esse processo é concluído.
...
- **A configuração é sua.** O engine e os shells são de código aberto (MPL-2.0). Sobreponha sua própria autenticação, telemetria ou CA; hospede ou não; você tem controle total de recursos e custos, rastreado no git, sem ficar preso a um banco de dados SaaS.
- **A governança pode ser dado, não um painel.** Quando você quiser esse controle, gerencie o catálogo de ferramentas como um repositório Git - a revisão de pull request vira aprovação de marca, com uma trilha de auditoria completa e rollback instantâneo de cada modelo que sua equipe pode tocar. É uma opção, não uma obrigação, e pertence exatamente a uma mesa: os criadores trabalham inteiramente dentro do aplicativo, salvando o que fazem como uma **sessão** e repassando isso como um link de compartilhamento, um backup ou uma colaboração ao vivo - nada disso precisa de git. Quando uma dessas sessões merece se tornar um ponto de partida permanente, quem administra a implantação abre o link, registra seus valores como um **modelo** nessa ferramenta no pacote de marca e faz o commit. A partir daí, ele aparece no seletor "New from template" da ferramenta e pode ser referenciado diretamente como `?template=<id>`. O Git é o passo de bloqueio do administrador, usado uma vez, e nunca algo que um criador precise tocar. Veja [Adoção e Governança](/info/adoption-governance.html).
- **As barreiras de proteção são estruturais.** As restrições de marca são fixadas nos modelos, não publicadas como diretrizes que as pessoas podem ignorar. O resultado errado não é desencorajado - ele é irrepresentável.
...
![Strip Hidden Data: o arquivo chega na tela e o selo declara claramente que nada é enviado](/t/url-shot?url=%2F%23%2Ftool%2Fstrip-data&width=1200&height=750&dpi=192&waitMs=2000&walker=1&format=svg&dark=1&filename=aud-strip-data)
...
![Prompt to Image - um bloco de texto de prompt composto em uma imagem quadrada, sem nada no resultado que não estivesse na entrada](/t/url-shot?url=%2F%23%2Ftool%2Fprompt-card%3Ffull&width=1440&height=900&dpi=192&waitMs=2200&walker=1&format=svg&cropSelector=%23tool-canvas&dark=1&filename=ov2-prompt-to-image)
...
## Independente, ou governado por um plano de controle

Duas formas, e você escolhe por implantação. **Independente é o padrão e não precisa de servidor:** o Lolly renderiza no dispositivo, cada produtor trabalha dentro do aplicativo, e a governança git-como-dado acima é totalmente opcional - uma única organização pode rodar este repositório sem nada hospedado. **Quando você quiser controle em toda a organização, adicione um plano de controle.** O [lolly.work](https://lolly.work) é um serviço separado, de código aberto (MPL-2.0), que você hospeda - ou avalia no sandbox hospedado - e que governa o shell em tempo real: login protegido por SSO, política de feature-flag / exportação / marca d'água, sobreposições de entrada de ferramenta, federação de catálogo, aprovações e um log de auditoria encadeado por hash, tudo servido ao shell sem nenhuma mudança de código aqui. Ele é agnóstico de marca (configuração mais a montagem de um pacote), consome o engine e os pacotes deste repositório sem modificação, e nunca se torna o caminho de renderização: o Lolly continua renderizando no dispositivo por design. OSS = liberdade individual; OSS + plano de controle = liberdade organizacional.
