...
向量工具(Brand Lockup、QR Code、Street Map、Wayfinding Signage、Pose Geeko、標誌組合、Diagram Builder)可匯出 **DXF** - 這是雷射切割機、貼紙繪圖機與 CNC/CAD 軟體都能讀取的 AutoCAD R12 交換格式。幾何圖形會以**毫米為單位的輪廓路徑**寫出(曲線會壓平至精細誤差範圍內),文字會轉為路徑輪廓,顏色則對應至最接近的 AutoCAD 色彩索引(通常用來決定切割機上的工具/操作)。DXF 僅限線稿 - 照片或濾鏡處理過的區域沒有對應的切割路徑形式,會被捨棄(Lolly 會提出警告),因此若需保留點陣內容,請改用 SVG/PDF。
...
**思考。**色彩運算是以知覺方式進行,而非直接對原始通道運算。調色盤衍生、漸變、和諧配色與對比度都在 **OKLCH/OKLab** 中執行,超出色域的顏色會透過 CSS Color 4 自身的色域對映演算法 - 帶知覺距離檢查的彩度縮減 - 拉回範圍內,而非直接裁切通道,因此鮮豔的顏色會落在你實際能接受的最接近顏色,而不是變得死板。漸層可依你選擇的空間內插(預設為 OKLab,或 `oklch`、`lab`、`lch`、`srgb`、`srgb-linear`、`hsl`,極座標空間還可指定色相走向),混色採**預先乘算(premultiplied)**,因此淡出至透明時會保持正確的顏色,而不會在過程中偏暗趨黑。同一套內插器同時服務預覽與匯出走查器 - 這正是過去圓錐漸層在螢幕上與匯出檔案中呈現不一致的問題所在。
