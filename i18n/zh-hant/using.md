...
> 第一次使用嗎？[快速上手](/info/quickstart.html) 讓你在幾分鐘內就能開始動手做，而 [Lolly 給維運人員](/info/operators.html) 說明如何安裝／部署本應用程式；本頁說明的是開啟後如何操作。
...
**目錄**（`#/c`，或選單中的**目錄**連結）匯集了你的工具能取用的一切——品牌標誌、圖片、音訊與動態內容，依類型分組——同時也是你**自己的創作檔案**存放的地方。沒有伺服器、沒有管理主控台、沒有 pull request：一切都在你的裝置上。

- **把你的檔案帶進來。** 把任何圖片、SVG、音訊片段、影片、Lottie 或 PDF 拖曳到上傳區——或點擊選取——它就會立刻進入你的目錄，在每個工具的素材選擇器中隨時可用。你想匯入多少都可以；它們絕不會離開你的裝置。
- **把常用的加入最愛。** 為某個素材（或某個品牌色票）按下 ★，它就會被釘選到每個選擇器的最上方，讓你最常用的標誌或色彩一鍵即達。
...
- **設計權杖（JSON）**、**CSS 變數**或 **CSS 類別**——把品牌直接放進樣式表或建置流程；
- **Adobe Swatch Exchange（.ase）**——載入 Illustrator 或 Photoshop；
- **GIMP 調色盤（.gpl）**——供 GIMP 或 Inkscape 使用。

**字型（Fonts）**面板會列出你的品牌字體，每個旁邊都有**下載**，可安裝到本機或交給印刷廠。（[品牌工作室](/info/brand-studio.html)的「色彩」分頁也提供相同的調色盤下載。）

素材是這條開放、自己動手做的路徑的一半；另一半則是**製作你自己的工具**——自由畫布（前述的版面工作室）讓你以視覺化方式打造工具，完全不需要寫程式。
...
該開關旁邊是 **Neurospicy 模式**——一段可選、令人平靜的背景專注音軌，會在你工作時輕柔播放。開啟它會在底部角落開啟一個小型的**播放器面板（player dock）**，並隨著你在應用程式各處移動而跟隨；你可以在其中搜尋並挑選音軌、往前或往後跳、設定音量，以及將它最小化或關閉。音軌清單涵蓋幾個類別——程序生成的 *Lolly Sings* 曲目、環境循環與節拍、你自己上傳的音訊，以及少數幾個即時網路**電台**（這些需要連線；其餘一切都能離線播放）。它**預設為關閉**，而且和聲音一樣，會跨工作階段與跨裝置被記住。關閉聲音也會同時將專注音軌靜音。
