...
- <!--i:trash--> **最乾淨的切換方式：**先執行**個人資料 → 儲存空間 → 清除我的所有資料**，再**匯入**你即將進入的那個情境的包裹。這樣你就完全以那份個人資料的身分創作。
- <!--i:layers--> **疊加：**如果沒有先清除就直接匯入，會**合併**——匯入的個人資料、工作階段與圖片會疊加在既有內容之上，同名的項目會被取代，其餘保持不變。適合把某個團隊已儲存的工作階段拉進你自己的環境；但如果你需要乾淨的角色界線，這就不是你要的做法。
- <!--i:monitor--> **並存：**由於一切都以裝置為範圍，不同的瀏覽器設定檔、不同的使用者帳號，或另外安裝的第二個 PWA，各自都會有自己獨立的 Lolly 個人資料。可以同時執行你個人的安裝和活動用資訊站的安裝，不必切換。
...
- <!--i:download--> **匯出我的資料**會下載一個 `LollyTools-<First>-<Last>-<YYYY-MM-DD>-<n>.zip` 檔——依所屬個人資料命名，並附上當日序號，避免重複匯出時互相衝突（若個人資料缺少對應欄位，該部分名稱就會省略）。裡面包含你的個人資料、每個已儲存的工作階段（含縮圖）、你上傳的圖片，以及你的偏好設定（主題、版面、本機活動統計）。
- <!--i:upload--> 另一台安裝上的**匯入資料…**會讀入那個檔案，讓你從離開的地方原封不動接續下去。
...
- <!--i:people--> **個人資料（Profile）**——*你*（或你的團隊，或你所扮演的角色）：姓名、聯絡方式、大頭照、你儲存的作品。屬於個人、存在裝置本機，可以包裹的形式攜帶。
- <!--i:palette--> **平台（Platform）**——*品牌*本身：每個工具據以呈現的顏色、字型與全域設定。共用且一致，不屬於個人。
- <!--i:sliders--> **功能（Capabilities）**——*應用程式能做什麼*：完整的功能集合，以及你可以使用的工具。
...
- <!--i:box--> **內容設定檔（Content profile）**——`profiles.json` 中的一項建置期設定，將一組工具包綁定到某個品牌目錄（例如 `suse`、`lolly-start`）。這是操作者部署時所選擇的項目，也是 `profile` **URL／CLI 參數**在匯出時用來選擇*色彩*版本的依據（ICC/CMYK 印刷條件——見[URL 模式](/info/url-mode.html)）。這兩者談的都是*建置／輸出*，而不是*你*。詳見[設定](/info/configuration.html)。
- <!--i:seal--> **身分設定檔（Identity profile）**——你可以選擇註冊的**已驗證 Content Credentials 身分**（一張將你的電子郵件與已簽署匯出檔綁定的短期憑證）。這是一種簽署身分，和個人資料的姓名／聯絡欄位是分開的，不過**使用我的資料**會決定兩者是否會被嵌入。詳見[Content Credentials 身分](/info/content-credentials-identity.html)。
