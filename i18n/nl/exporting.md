...
Vectortools (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, de logo-lockups, Diagram Builder) kunnen **DXF** exporteren - het AutoCAD R12-uitwisselingsformaat dat lasersnijders, vinylplotters en CNC-/CAD-software lezen. Geometrie wordt geschreven als omtrek**paden in millimeters** (curven afgevlakt tot een fijne tolerantie), tekst wordt omgezet in paden en kleur komt terecht op de dichtstbijzijnde AutoCAD Color Index (die doorgaans de tool/bewerking op een snijmachine aanstuurt). DXF is alleen lijnkunst - een fotografisch of gefilterd gebied heeft geen snijpadvorm en wordt weggelaten (Lolly waarschuwt), gebruik dus SVG/PDF wanneer je rasterinhoud wilt behouden.
...
**Denken.** Kleurwiskunde gebeurt perceptueel in plaats van in ruwe kanalen. Paletafleiding, verlopen, harmonieën en contrast draaien in **OKLCH/OKLab**, en een kleur buiten het gamut wordt binnen bereik gebracht door het eigen gamut-mapping-algoritme van CSS Color 4 - chromareductie met een perceptuele-afstandscontrole - in plaats van door kanalen af te knippen, zodat een levendige kleur uitkomt op de dichtstbijzijnde kleur die je daadwerkelijk zou accepteren in plaats van een afgevlakte. Gradiënten interpoleren in een ruimte die je kiest (standaard OKLab, of `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, met een tint-reisrichting voor de polaire) en mengen is **voorvermenigvuldigd (premultiplied)**, zodat een vervaging naar transparant de juiste kleur behoudt in plaats van onderweg naar zwart te verdonkeren. Eén interpolator bedient zowel de preview als de exportwalkers - wat voorkwam dat een conische gradiënt op het scherm op de ene manier en in het geëxporteerde bestand op een andere manier werd gemengd.
