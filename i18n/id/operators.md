...
> **Kondisi saat ini.** Properti keamanan Lolly kuat sejak dari desain, dan engine kriptografi serta file-parsing-nya sedang menjalani hardening infrastruktur setingkat enterprise milik SUSE. Segel, penandatanganan on-device dan enkripsi di bawah ini nyata dan dapat dipertahankan sekarang, dan sedang matang menuju sertifikasi independen - jadi di mana sebuah kontrak menuntut jaminan bersertifikat, terapkan ini sebagai defence-in-depth selagi proses itu selesai.
...
- **Konfigurasi adalah milik Anda.** Engine dan shell bersifat open source (MPL-2.0). Overlay auth, telemetri atau CA Anda sendiri; hosting sendiri atau tidak; Anda memegang kendali penuh atas fitur dan biaya, git-tracked, tidak terkunci dalam database SaaS.
- **Governance bisa berupa data, bukan dashboard.** Ketika Anda menginginkan kendali itu, kelola katalog alat sebagai sebuah repositori Git - review pull-request menjadi persetujuan brand, dengan audit trail lengkap dan rollback instan pada tiap templat yang bisa disentuh tenaga kerja Anda. Ini sebuah opsi, bukan kewajiban, dan itu menjadi tanggung jawab satu meja saja: kreator bekerja sepenuhnya di dalam aplikasi, menyimpan apa yang mereka buat sebagai sebuah **sesi** dan meneruskannya sebagai link berbagi, sebuah backup atau sebuah kolaborasi live - tidak satu pun dari itu memerlukan git. Ketika salah satu sesi itu layak menjadi titik awal permanen, siapa pun yang menjalankan deployment membuka link itu, mencatat nilai-nilainya sebagai sebuah **templat** pada alat itu di brand pack lalu melakukan commit. Sejak itu ia muncul di chooser "New from template" milik alat itu dan dapat di-deep-link sebagai `?template=<id>`. Git adalah langkah pengunci milik admin, digunakan sekali, dan bukan sesuatu yang harus disentuh oleh seorang kreator. Lihat [Adoption & Governance](/info/adoption-governance.html).
- **Guard-rail bersifat struktural.** Batasan brand di-hard-code ke dalam templat, bukan dipublikasikan sebagai pedoman yang bisa diabaikan orang. Output yang salah bukan sekadar tidak dianjurkan - ia tidak dapat direpresentasikan sama sekali.
...
![Strip Hidden Data: file itu masuk ke kanvas dan badge-nya menyatakan dengan jelas bahwa tidak ada yang diunggah](/t/url-shot?url=%2F%23%2Ftool%2Fstrip-data&width=1200&height=750&dpi=192&waitMs=2000&walker=1&format=svg&dark=1&filename=aud-strip-data)
...
![Prompt to Image - sebuah blok teks prompt yang di-typeset ke dalam gambar persegi, tanpa sesuatu pun di hasilnya yang tidak ada di input](/t/url-shot?url=%2F%23%2Ftool%2Fprompt-card%3Ffull&width=1440&height=900&dpi=192&waitMs=2200&walker=1&format=svg&cropSelector=%23tool-canvas&dark=1&filename=ov2-prompt-to-image)
...
## Standalone, atau dikelola oleh sebuah control plane

Dua bentuk, dan Anda memilih per deployment. **Standalone adalah default dan tidak memerlukan server:** Lolly merender di perangkat, tiap produser bekerja di dalam aplikasi, dan governance git-as-data di atas sepenuhnya opsional - satu organisasi saja bisa menjalankan repo ini tanpa apa pun yang di-hosting sama sekali. **Ketika Anda menginginkan kendali di seluruh organisasi, tambahkan sebuah control plane.** [lolly.work](https://lolly.work) adalah layanan terpisah, open-source (MPL-2.0) yang Anda hosting sendiri - atau Anda evaluasi di sandbox yang di-hosting - yang mengelola shell secara live: sign-in yang di-gate SSO, kebijakan feature-flag / ekspor / watermark, overlay tool-input, federasi katalog, persetujuan dan sebuah audit log berantai-hash, semuanya disajikan ke shell tanpa perubahan kode di sini. Ini brand-agnostic (config plus sebuah pack mount), menggunakan engine dan pack repo ini tanpa modifikasi, dan tidak pernah menjadi render path: Lolly tetap merender di perangkat sejak dari desain. OSS = kebebasan individu; OSS + control plane = kebebasan organisasi.
