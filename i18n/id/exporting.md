...
Alat vektor (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, lockup logo, Diagram Builder) dapat mengekspor **DXF** - format pertukaran AutoCAD R12 yang dibaca oleh laser cutter, plotter vinil dan perangkat lunak CNC/CAD. Geometri ditulis sebagai **path garis luar dalam milimeter** (kurva diratakan hingga toleransi halus), teks di-outline menjadi path dan warna dipetakan ke AutoCAD Color Index terdekat (yang biasanya menentukan alat/operasi pada mesin pemotong). DXF hanya berupa line-art - area fotografis atau berfilter tidak memiliki bentuk cut-path dan akan dibuang (Lolly memberi peringatan), jadi gunakan SVG/PDF saat Anda perlu mempertahankan konten raster.
...
**Berpikir.** Matematika warna berjalan secara perseptual, bukan pada kanal mentah. Derivasi palet, ramp, harmoni dan kontras berjalan dalam **OKLCH/OKLab**, dan warna yang di luar gamut dibawa ke dalam rentang oleh algoritma pemetaan gamut milik CSS Color 4 sendiri - reduksi chroma dengan pemeriksaan jarak perseptual - bukan dengan memotong kanal, sehingga warna yang mencolok mendarat pada warna terdekat yang benar-benar akan Anda terima, bukan warna yang diratakan. Gradient berinterpolasi dalam ruang yang Anda pilih (OKLab secara default, atau `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, dengan arah perjalanan hue untuk yang polar), dan pencampuran bersifat **premultiplied**, sehingga fade ke transparan tetap warna yang benar alih-alih menggelap ke hitam di tengah jalan. Satu interpolator melayani baik pratinjau maupun walker ekspor - yang mana itulah yang menghentikan gradient conic dari diblend dengan cara berbeda di layar dibanding di file yang diekspor.
