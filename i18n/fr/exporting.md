...
Les outils vectoriels (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, les lockups de logo, Diagram Builder) peuvent exporter en **DXF** - le format d'échange AutoCAD R12 que lisent les découpeuses laser, les traceurs vinyle et les logiciels CNC/CAO. La géométrie est écrite sous forme de **chemins de contour en millimètres** (courbes aplaties à une tolérance fine), le texte est converti en chemins et la couleur atterrit sur l'index de couleur AutoCAD le plus proche (qui pilote généralement l'outil/l'opération sur une découpeuse). Le DXF n'est que du trait - une zone photographique ou filtrée n'a pas de forme de chemin de découpe et est abandonnée (Lolly le signale), donc utilise SVG/PDF quand tu dois conserver du contenu raster.
...
**Pensée.** Les calculs de couleur se font de manière perceptuelle plutôt que sur des canaux bruts. La dérivation de palette, les rampes, les harmonies et le contraste tournent en **OKLCH/OKLab**, et une couleur hors gamme est ramenée dans la plage par l'algorithme propre de mise en correspondance de gamme de CSS Color 4 - une réduction de chroma avec un contrôle de distance perceptuelle - plutôt que par un écrêtage de canaux, si bien qu'une couleur vive atterrit sur la couleur la plus proche que tu accepterais réellement au lieu d'une couleur aplatie. Les dégradés interpolent dans un espace que tu choisis (OKLab par défaut, ou `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, avec une direction de parcours de teinte pour les espaces polaires), et le mélange est **prémultiplié**, si bien qu'un fondu vers la transparence garde la bonne couleur au lieu de s'assombrir vers le noir en chemin. Un seul interpolateur sert à la fois l'aperçu et les walkers d'export - ce qui a mis fin au fait qu'un dégradé conique soit mélangé d'une façon à l'écran et d'une autre dans le fichier exporté.
