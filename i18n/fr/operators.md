...
> **Où ça en est aujourd'hui.** Les propriétés de sécurité de Lolly sont solides par conception, et ses moteurs de cryptographie et d'analyse de fichiers passent actuellement par le durcissement d'infrastructure de niveau entreprise de SUSE. Les sceaux, la signature sur l'appareil et le chiffrement décrits ci-dessous sont réels et défendables dès maintenant, et progressent vers une certification indépendante - donc là où un contrat exige une assurance certifiée, déploie-les comme défense en profondeur pendant que ce processus se termine.
...
- **La configuration t'appartient.** Le moteur et les shells sont open source (MPL-2.0). Superpose ta propre authentification, ta télémétrie ou ton AC ; héberge-le ou non ; tu gardes le contrôle total des fonctionnalités et des coûts, suivi par git, sans enfermement dans une base de données SaaS.
- **La gouvernance peut être des données, pas un tableau de bord.** Quand tu veux ce contrôle, gère le catalogue d'outils comme un dépôt Git - la revue de pull request devient l'approbation de marque, avec une piste d'audit complète et un retour arrière instantané de chaque template accessible à tes équipes. C'est une option, pas une obligation, et elle repose sur un seul bureau : les créateurs travaillent entièrement dans l'application, enregistrant ce qu'ils font comme une **session** et la transmettant sous forme de lien de partage, de sauvegarde ou de collaboration en direct - rien de tout cela n'a besoin de git. Quand une de ces sessions mérite de devenir un point de départ permanent, quiconque gère le déploiement ouvre le lien, enregistre ses valeurs comme **template** sur cet outil dans le pack de marque, et committe. À partir de là il apparaît dans le sélecteur "New from template" de l'outil et devient accessible par lien profond via `?template=<id>`. Git est l'étape de verrouillage de l'administrateur, utilisée une fois, et jamais quelque chose qu'un créateur doit toucher. Voir [Adoption et gouvernance](/info/adoption-governance.html).
- **Les garde-fous sont structurels.** Les contraintes de marque sont codées en dur dans les templates, pas publiées comme des directives que l'on peut ignorer. Le mauvais résultat n'est pas découragé - il est irreprésentable.
...
![Strip Hidden Data : le fichier arrive sur le canevas et le badge indique clairement que rien n'est envoyé](/t/url-shot?url=%2F%23%2Ftool%2Fstrip-data&width=1200&height=750&dpi=192&waitMs=2000&walker=1&format=svg&dark=1&filename=aud-strip-data)
