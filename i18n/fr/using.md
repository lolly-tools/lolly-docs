...
![La vue divisée d'un outil - la pile de contrôles à gauche, et l'histogramme groupé en direct qu'il dessine à droite](/t/url-shot?url=%2F%23%2Ftool%2Fchart%3Fct%3Dbar%26t%3DExample%2520data%26st%3DSample%2520values%252C%2520not%2520a%2520real%2520dataset%26d%3DMonth%252CSeries%2520A%252CSeries%2520B%252CSeries%2520C%250AJan%252C12%252C9%252C5%250AFeb%252C18%252C14%252C7%250AMar%252C24%252C17%252C11%250AApr%252C29%252C23%252C15%26lg%3D1&width=1440&height=900&dpi=192&waitMs=2500&walker=1&format=svg&dark=1&filename=vt-tool-split-view)
...
- <!--i:upload--> **Importe tes fichiers.** Glisse une image, un SVG, un extrait audio, une vidéo, un Lottie, un PDF ou une présentation PowerPoint sur la zone d'import - ou clique pour choisir - et il arrive instantanément dans ton catalogue, prêt dans le sélecteur de ressources de chaque outil. Un PDF multipage ou un `.pptx` te demande quelles pages ou diapositives conserver - chacune devient une ressource SVG. Importe autant que tu veux ; rien ne quitte jamais ton appareil.
- <!--i:star--> **Mets en favori ce que tu utilises souvent.** Mets une ★ sur une ressource (ou une teinte de marque) et elle s'épingle en haut de chaque sélecteur, pour que ton logo ou ta couleur de prédilection soit à un clic.
- <!--i:folder--> **Fais le ménage.** Recatégorise une ressource dans un autre groupe, masque une ressource de marque partagée que tu n'utilises pas (avec **Show hidden** pour la faire réapparaître) ou supprime carrément tes propres imports. Le même geste de sélection multiple et la même barre d'actions flottante que dans Projets fonctionnent ici aussi, donc tout cela peut se faire sur toute une sélection à la fois.
- <!--i:layers--> **Détache une vidéo de son arrière-plan.** Ouvre le détail d'une vidéo ou fais un clic droit sur sa carte dans n'importe quel sélecteur de ressources et choisis **Remove background…** pour enregistrer une variante transparente - un WebP ou un PNG animé avec une vraie transparence alpha. Choisis une **Method** : un **On-device model** découpe un sujet d'une scène chargée, ou une **Colour key** détoure un fond uni et bien éclairé comme un fond vert ou un mur plat, avec **Tolerance**, **Softness** et **Spill removal** pour affiner le contour. La détection par couleur ne nécessite ni téléchargement de modèle ni réseau, donc **Remove background** est proposé sur toute vidéo et donne souvent un résultat plus net sur des images bien cadrées. Un contrôle **Resolution** (360, 480, 720 ou 1080p, jamais au-delà de la source) échange du détail contre un fichier plus petit et plus rapide. Il s'exécute en tâche de fond sur ton appareil. Le découpage terminé arrive à côté de l'original comme sa propre ressource, et le Content Credential de la vidéo source l'accompagne comme ingrédient. (Voir [Généré une fois, restitué à l'identique](/info/ai-features.html) pour comprendre pourquoi retirer un arrière-plan reste une simple modification.)
