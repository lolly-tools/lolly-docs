...
**Una estrategia de prevención de pérdida de datos y procedencia a prueba de futuro y en profundidad, disfrazada de plataforma creativa.**
...
> **Hablemos claro primero.** Las propiedades de seguridad de Lolly son sólidas *por diseño*, y sus motores de criptografía y análisis de archivos están actualmente pasando por el estricto proceso de fortalecimiento de infraestructura de SUSE, preparándose para escala empresarial — se nos da realmente bien esto. Los sellos, la firma en el dispositivo y el cifrado descritos abajo son reales y defendibles; mientras ese fortalecimiento se completa, trátalos como defensa en profundidad en lugar de un control certificado allí donde se requiera contractualmente una garantía independiente. Preferimos que lo sepas desde el principio.
...
- **La gobernanza es datos, no un panel de control.** El catálogo de herramientas es la fuente de verdad, gestionado como un repositorio Git — la revisión de pull requests *es* la moderación, y obtienes un registro de auditoría completo y reversión instantánea de cada plantilla que tu personal pueda tocar. Consulta [Adopción y Gobernanza](/info/adoption-governance.html).
...
Las exportaciones pueden llevar **Content Credentials** — un manifiesto [C2PA](https://c2pa.org) firmado y vinculado a un hash de los bytes del archivo. Esto es **con *evidencia* de manipulación, no *a prueba* de manipulación**: no impide que nadie altere un archivo, pero cualquier cambio posterior rompe el sello y un verificador compatible con C2PA lo detecta. Esa es la propiedad honesta y útil — puedes *detectar* la alteración, de forma criptográfica y sin conexión.
...
- **Niveles de confianza.** Una exportación sin inscribir es estructuralmente válida pero está firmada de forma anónima (`untrusted`). Inscribe una **identidad verificada** (certificado de corta duración de la CA de Lolly, vinculado a un correo electrónico) y los verificadores que confían en la raíz de Lolly reportarán `trusted` + el correo electrónico del firmante. Una autoridad de sellado de tiempo confiable y el visto bueno de un validador externo (conformidad C2PA) están en la hoja de ruta, no publicados todavía — los niveles se etiquetan con honestidad y un archivo nunca muestra un verde falso.
...
> **Carencia conocida, dicha sin rodeos:** el verificador de Lolly todavía no lee por completo los manifiestos de reclamación **v2** de C2PA de otros productores; y WebM lleva el manifiesto como un adjunto de Matroska (todavía no existe una asignación C2PA estandarizada para WebM), así que las herramientas de terceros verifican el MP4 de Lolly pero no su WebM.
...
Lolly está diseñado para funcionar **sin red en el momento del renderizado**. El shell web es una PWA offline-first (service worker); las fuentes y WASM se almacenan en el dispositivo; el estado de las herramientas se persiste localmente a través del puente del host, nunca con `localStorage`. Cualquier herramienta que acceda a la red solo puede hacerlo mediante una capacidad `host.net` **incluida en una lista blanca**, que debe declarar en su manifiesto — un shell que no pueda (o no quiera) cumplirla la neutraliza. Así, una instalación completamente aislada (air-gapped) renderiza, exporta, cifra y verifica credenciales sin nada a lo que reportarse.

## Lo que debes saber antes de confiar en esto

Los operadores merecen conocer las salvedades, no solo las afirmaciones:

- **Fortalecimiento para escala empresarial.** Como se indicó al principio — la criptografía y los analizadores de archivos están actualmente pasando por el estricto fortalecimiento de infraestructura de SUSE para escala empresarial; sólidos por diseño, y trátalos como defensa en profundidad allí donde se requiera contractualmente una garantía independiente.
- **Los hooks de las herramientas *no* son un sandbox de seguridad.** El `hooks.js` opcional de una herramienta se ejecuta con el puente del host inyectado, pero en un shell de navegador se ejecuta en el ámbito de la página y *puede* acceder a `window`/`document`/`fetch`. Trata el código de las herramientas igual que tratarías cualquier código que ejecutas — revísalo. Por eso importa el modelo de catálogo-como-revisión-en-Git, y por eso las herramientas de terceros no confiables no deberían ejecutarse hasta que se publique el aislamiento por Worker.
- **C2PA es a prueba de detección de manipulación, no a prueba de manipulación**, y las carencias mencionadas antes sobre la lectura de v2 y WebM son reales.
- **Los niveles de cifrado son distintos.** Los bloqueos *Estándar* son disuasivos; solo *Fuerte* (AES-256) es protección real, y los archivos Fuertes no se abren en todos los lectores heredados.
