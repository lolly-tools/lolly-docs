...
> **Dónde está hoy.** Las propiedades de seguridad de Lolly son sólidas por diseño, y sus motores de criptografía y de análisis de archivos están pasando por el endurecimiento de infraestructura de nivel empresarial de SUSE. Los sellos, la firma en el dispositivo y el cifrado descritos abajo son reales y defendibles ya, y avanzan hacia la certificación independiente - así que, cuando un contrato exija garantía certificada, despliégalos como defensa en profundidad mientras ese proceso se completa.
...
- **La configuración es tuya.** El motor y los shells son de código abierto (MPL-2.0). Superpón tu propia autenticación, telemetría o CA; alójalo o no; tienes control total de funciones y costes, con seguimiento en git, sin quedar atado a una base de datos SaaS.
- **La gobernanza puede ser datos, no un panel.** Cuando quieras ese control, gestiona el catálogo de herramientas como un repositorio Git - la revisión de pull requests se convierte en aprobación de marca, con un registro de auditoría completo y una reversión instantánea de cada plantilla que tu equipo puede tocar. Es una opción, no una obligación, y recae en un único puesto: quienes crean trabajan por completo dentro de la app, guardando lo que hacen como una **sesión** y transmitiéndolo como un enlace para compartir, una copia de seguridad o una colaboración en vivo - nada de eso necesita git. Cuando una de esas sesiones merece convertirse en un punto de partida permanente, quien gestiona el despliegue abre el enlace, registra sus valores como una **plantilla** de esa herramienta en el paquete de marca y hace commit. A partir de ahí aparece en el selector "Nuevo desde plantilla" de la herramienta y se puede enlazar directamente como `?template=<id>`. Git es el paso de bloqueo del administrador, usado una sola vez, y nunca algo que quien crea tenga que tocar. Consulta [Adopción y gobernanza](/info/adoption-governance.html).
- **Las barreras de protección son estructurales.** Las restricciones de marca están codificadas en las plantillas, no publicadas como directrices que se pueden ignorar. El resultado incorrecto no se desaconseja - es irrepresentable.
...
![Strip Hidden Data: el archivo llega al lienzo y la insignia declara claramente que no se sube nada](/t/url-shot?url=%2F%23%2Ftool%2Fstrip-data&width=1200&height=750&dpi=192&waitMs=2000&walker=1&format=svg&dark=1&filename=aud-strip-data)
...
![Prompt to Image - un bloque de texto de prompt compuesto en una imagen cuadrada, sin nada en el resultado que no estuviera en la entrada](/t/url-shot?url=%2F%23%2Ftool%2Fprompt-to-image%3Ffull&width=1440&height=900&dpi=192&waitMs=2200&walker=1&format=svg&cropSelector=%23tool-canvas&dark=1&filename=ov2-prompt-to-image)
