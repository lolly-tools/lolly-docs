...
|---|---|---|---|
| Sincronización del catálogo de herramientas | Nada personal - una solicitud al propio índice público de herramientas y assets de Lolly, hacia el origen de la propia app | Al iniciar, y luego en caché sin conexión | La app sigue funcionando con su conjunto de herramientas en caché. Solo deja de descubrir herramientas nuevas |
| Una herramienta que necesita datos en directo | Lo que esa herramienta concreta solicite, al host que se nombra en su propia descripción. Hoy eso es solo la búsqueda de ciudad en la herramienta Meeting Planner, que le pide a `geocoding-api.open-meteo.com` que convierta un nombre de ciudad en coordenadas y una zona horaria - sin cuenta, sin clave y sin ningún identificador más allá de la propia solicitud. La entrada lo indica justo donde escribes, y cada respuesta se guarda en tu dispositivo para que una ciudad solo se busque una vez | Solo mientras usas esa herramienta, y solo cuando introduces una ubicación | Solo falla esa búsqueda. Puedes seguir escribiendo coordenadas a mano, y nada más se ve afectado |
| Google Fonts | El nombre de la familia de fuente elegida y tu dirección IP, a los servidores de fuentes de Google (`fonts.googleapis.com` para la hoja de estilos, `fonts.gstatic.com` para el archivo de fuente) | Solo si añades una fuente de Google en el editor de marca, **y solo después de aceptarlo en un cuadro de diálogo que dice exactamente eso** - una descarga única por familia, que luego vive en tu dispositivo y se usa sin conexión | El selector de Google Fonts falla de forma segura (queda desactivado). Sube un archivo de fuente en su lugar |
| Enviar a Google Drive | El único archivo que elegiste enviar, a la API de Drive de Google (`www.googleapis.com`), tras un inicio de sesión de Google que completas en la propia ventana emergente de Google. El acceso de Lolly se limita a los archivos que ha creado (el ámbito `drive.file` - nunca puede leer el resto de tu Drive), y el token de sesión se guarda en memoria solo durante la sesión, nunca almacenado | Solo cuando pulsas "Enviar a Google Drive" en una exportación EMF, y solo en compilaciones donde el operador ha configurado un id de cliente de Google - sin uno, el botón no existe | El botón nunca aparece. Descarga el archivo y súbelo tú mismo a Drive |
| Enviar a Dropbox | El único archivo que elegiste enviar, a la API de Dropbox (`api.dropboxapi.com` para el inicio de sesión y los metadatos, `content.dropboxapi.com` para el archivo en sí), tras un inicio de sesión de Dropbox que completas en la propia ventana de Dropbox. El acceso de Lolly se limita a la carpeta de la app (solo puede ver `Apps/` y su propia carpeta ahí dentro - nunca el resto de tu Dropbox), el enlace "Abrir" que te muestra es un enlace privado de corta duración (no se crea ningún elemento público compartido), y un token de renovación solo se guarda si marcas "Permanecer conectado" | Solo cuando pulsas "Enviar a Dropbox" en un archivo, y solo en compilaciones donde el operador ha configurado un id de cliente de Dropbox - sin uno, el botón no existe | El botón nunca aparece. Descarga el archivo y súbelo tú mismo a Dropbox |
| Enviar a OneDrive | El único archivo que elegiste enviar, a los servicios de identidad y Graph de Microsoft (`login.microsoftonline.com` para el inicio de sesión, `graph.microsoft.com` para la subida; un archivo grande se sube por partes a una dirección de subida propiedad de Microsoft en `api.onedrive.com`, `*.up.1drv.com` o `*.sharepoint.com`), tras un inicio de sesión de Microsoft que completas en la propia ventana de Microsoft. El acceso de Lolly se limita a su propia carpeta bajo `Apps/` (nunca puede leer el resto de tu OneDrive), más tu nombre visible para la etiqueta de la cuenta, y un token de renovación solo se guarda si marcas "Permanecer conectado" | Solo cuando pulsas "Enviar a OneDrive" en un archivo, y solo en compilaciones donde el operador ha configurado un id de cliente de Microsoft - sin uno, el botón no existe | El botón nunca aparece. Descarga el archivo y súbelo tú mismo a OneDrive |
| Perfiles de impresión ICC | Nada personal - una solicitud de un perfil de condición de impresión estándar, al registro público del ICC (`registry.color.org`, `www.color.org`) | Solo si haces clic en un preajuste ICC en el gestor de perfiles de impresión - una descarga única por perfil, que luego vive en tu dispositivo | Los preajustes ICC fallan. Aporta tu propio perfil `.icc` en su lugar |
| Radio por internet | Nada personal - una solicitud de lista de reproducción y un flujo de audio, a la emisora (`api.somafm.com` y el servidor icecast que nombra, `*.somafm.com`) | Solo mientras reproduces la radio integrada opcional en el reproductor de sonido | La radio falla. Todas las demás funciones de sonido siguen funcionando |
| Una URL que le pides a una herramienta que capture | Una solicitud a la dirección web exacta que escribes, desde la herramienta de captura de URL. Sea cual sea esa dirección. Este host no aparece en la política de abajo, porque tú lo eliges en el momento de usarla | Solo cuando introduces una URL en esa herramienta e inicias la captura | Un operador no puede incluirlo en una lista blanca por host. Para eliminarlo, elimina la herramienta |
| Verificación de firma SEAL | **Nada.** La app web no tiene ningún resolutor DNS - ver más abajo | Nunca | Nada que bloquear |
| Modelos de detección de escaneo profundo | Nada personal - una descarga única de modelo desde el mismo origen (no un tercero) | Solo si activas el escaneo profundo de Verify | El escaneo profundo no está disponible. La verificación estándar sigue funcionando |
| Instancia remota | Lo que sea que devuelva la instancia que nombres, mediante la misma sincronización de catálogo descrita arriba - más una etiqueta de versión en las solicitudes que se le hacen (tipo de shell y versión del motor, la misma información que lleva un user agent), para que su operador pueda ver qué versiones de Lolly están en uso. En una instancia gestionada, mientras tienes la sesión iniciada, esa etiqueta también lleva un id de instalación propio del dispositivo, para que la lista de dispositivos del operador pueda distinguir esta instalación. Solo viaja con solicitudes que tu propio uso ya genera - no hay temporizador y nada se comunica por su cuenta - y salir de la instancia elimina el id, así que un dispositivo que se reconecta más tarde presenta uno nuevo. Tú eliges el host en el momento de usarlo, así que no aparece en la política de abajo | Solo si diriges explícitamente el shell a otro despliegue de Lolly | El cambio de instancia falla. Tu instancia local no se ve afectada |
