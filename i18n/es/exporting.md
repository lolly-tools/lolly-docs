...
Las herramientas vectoriales (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, los lockups de logo, Diagram Builder) pueden exportar **DXF** - el formato de intercambio AutoCAD R12 que leen cortadoras láser, plotters de vinilo y software CNC/CAD. La geometría se escribe como **rutas de contorno en milímetros** (curvas aplanadas a una tolerancia fina), el texto se convierte en contornos a rutas y el color se asigna al Color Index de AutoCAD más cercano (que típicamente controla la herramienta/operación en una cortadora). DXF es solo arte lineal - una región fotográfica o filtrada no tiene forma de ruta de corte y se descarta (Lolly avisa), así que usa SVG/PDF cuando necesites conservar contenido rasterizado.
...
**Pensamiento.** Las matemáticas de color suceden perceptualmente en lugar de en canales crudos. La derivación de paletas, las rampas, las armonías y el contraste corren en **OKLCH/OKLab**, y un color fuera de gamut se lleva al rango mediante el propio algoritmo de mapeo de gamut de CSS Color 4 - reducción de croma con una comprobación de distancia perceptual - en lugar de recortar canales, así que un color vivo aterriza en el color más cercano que realmente aceptarías en lugar de uno aplanado. Los degradados interpolan en un espacio que eliges (OKLab por defecto, o `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, con una dirección de recorrido de matiz para los polares), y la mezcla es **premultiplicada**, así que un desvanecido a transparente mantiene el color correcto en lugar de oscurecerse hacia negro en el camino. Un solo interpolador sirve tanto la vista previa como los walkers de exportación - que es lo que evitó que un degradado cónico se mezclara de una forma en pantalla y de otra en el archivo exportado.
