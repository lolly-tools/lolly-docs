...
Vektorverktyg (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, logotypkombinationerna, Diagram Builder) kan exportera **DXF** - AutoCAD R12-utbytesformatet som laserskärare, vinylplottrar och CNC/CAD-program läser. Geometri skrivs som konturbanor (**paths**) i millimeter (kurvor plattas till en fin tolerans), text konturas till banor och färg landar som närmaste AutoCAD Color Index (som vanligtvis styr verktyget/operationen på en skärmaskin). DXF är endast strecklinjekonst - ett fotografiskt eller filtrerat område har ingen skärbaneform och tas bort (Lolly varnar), så använd SVG/PDF när du behöver behålla rasterinnehåll.
...
**Tänkande.** Färgmatematik sker perceptuellt snarare än i råa kanaler. Palettderivering, ramper, harmonier och kontrast körs i **OKLCH/OKLab**, och en färg utanför gamut förs in i intervallet av CSS Color 4:s egen gamutmappningsalgoritm - kromareduktion med en perceptuell-avstånds-kontroll - snarare än genom att klippa kanaler, så en klar färg landar på den närmaste färg du faktiskt skulle acceptera i stället för en utplattad. Gradienter interpolerar i en rymd du väljer (OKLab som standard, eller `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, med en nyansresa-riktning för de polära), och blandning är **förmultiplicerad**, så en tona-till-transparent behåller rätt färg i stället för att mörkna mot svart på vägen. En enda interpolator betjänar både förhandsvisningen och exportgångarna - vilket är vad som hindrade en konisk gradient från att blandas ett sätt på skärmen och ett annat i den exporterade filen.
