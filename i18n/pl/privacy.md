...
|---|---|---|---|
| Synchronizacja katalogu narzędzi | Nic osobistego - zapytanie o własny, publiczny indeks narzędzi i zasobów Lolly, kierowane do własnego serwera aplikacji | Przy starcie, potem buforowane offline | Aplikacja działa na zbuforowanym zestawie narzędzi. Przestaje jedynie odkrywać nowe narzędzia |
| Narzędzie wymagające danych na żywo | To, o co poprosi dane konkretne narzędzie, kierowane do hosta wymienionego w jego własnym opisie. Obecnie jest to wyłącznie wyszukiwanie miasta w narzędziu Meeting Planner, które pyta `geocoding-api.open-meteo.com`, aby zamienić nazwę miasta na współrzędne i strefę czasową - bez konta, bez klucza i bez żadnego identyfikatora poza samym zapytaniem. Pole wejściowe informuje o tym dokładnie tam, gdzie wpisujesz tekst, a każda odpowiedź jest zapisywana na Twoim urządzeniu, więc dane miasto jest wyszukiwane tylko raz | Tylko podczas korzystania z tego narzędzia i tylko po wpisaniu lokalizacji | To jedno wyszukiwanie się nie powiedzie. Nadal możesz wpisać współrzędne ręcznie, a nic innego nie zostaje naruszone |
| Google Fonts | Nazwa wybranej rodziny czcionek oraz Twój adres IP, wysyłane do serwerów czcionek Google (`fonts.googleapis.com` dla arkusza stylów, `fonts.gstatic.com` dla pliku czcionki) | Tylko jeśli dodasz czcionkę Google w edytorze marki, **i tylko po zaakceptowaniu tego w oknie dialogowym, które mówi dokładnie to** - jednorazowe pobranie dla każdej rodziny, potem czcionka pozostaje na Twoim urządzeniu i jest używana offline | Wybór czcionek Google zawodzi bezpiecznie. Zamiast tego prześlij plik czcionki |
| Wyślij do Google Drive | Jeden wybrany przez Ciebie plik, wysyłany do API Google Drive (`www.googleapis.com`), po zalogowaniu do Google, które kończysz w oknie wyskakującym samego Google. Dostęp Lolly jest ograniczony do plików, które sam utworzył (zakres `drive.file` - nigdy nie może odczytać reszty Twojego Dysku), a token logowania jest przechowywany tylko w pamięci na czas sesji i nigdy nie jest zapisywany | Tylko po naciśnięciu "Send to Google Drive" przy eksporcie EMF i tylko w wersjach, w których operator skonfigurował identyfikator klienta Google - bez niego ten przycisk nie istnieje | Przycisk nigdy się nie pojawia. Pobierz plik i sam prześlij go na Dysk |
| Wyślij do Dropbox | Jeden wybrany przez Ciebie plik, wysyłany do API Dropbox (`api.dropboxapi.com` do logowania i metadanych, `content.dropboxapi.com` dla samego pliku), po zalogowaniu do Dropbox, które kończysz we własnym oknie Dropbox. Dostęp Lolly ogranicza się wyłącznie do folderu aplikacji (widzi tylko `Apps/` i swój własny folder w nim - nigdy resztę Twojego Dropboksa), pokazywany link "Open" to krótkotrwały link prywatny (nie tworzy się żadne publiczne udostępnienie), a token odświeżania jest zapisywany tylko wtedy, gdy zaznaczysz "stay connected" | Tylko po naciśnięciu "Send to Dropbox" przy pliku i tylko w wersjach, w których operator skonfigurował identyfikator klienta Dropbox - bez niego ten przycisk nie istnieje | Przycisk nigdy się nie pojawia. Pobierz plik i sam prześlij go do Dropboksa |
| Wyślij do OneDrive | Jeden wybrany przez Ciebie plik, wysyłany do usług tożsamości i Graph firmy Microsoft (`login.microsoftonline.com` do logowania, `graph.microsoft.com` do przesyłania; duży plik jest przesyłany fragmentami na adres przesyłania należący do Microsoftu pod `api.onedrive.com`, `*.up.1drv.com` lub `*.sharepoint.com`), po zalogowaniu do Microsoftu, które kończysz we własnym oknie Microsoftu. Dostęp Lolly jest ograniczony do własnego folderu pod `Apps/` (nigdy nie może odczytać reszty Twojego OneDrive) oraz Twojej nazwy wyświetlanej do etykiety konta, a token odświeżania jest zapisywany tylko wtedy, gdy zaznaczysz "stay connected" | Tylko po naciśnięciu "Send to OneDrive" przy pliku i tylko w wersjach, w których operator skonfigurował identyfikator klienta Microsoft - bez niego ten przycisk nie istnieje | Przycisk nigdy się nie pojawia. Pobierz plik i sam prześlij go do OneDrive |
| Profile drukarskie ICC | Nic osobistego - zapytanie o standardowy profil warunków druku, kierowane do publicznego rejestru ICC (`registry.color.org`, `www.color.org`) | Tylko po kliknięciu gotowego profilu ICC w menedżerze profili druku - jednorazowe pobranie dla każdego profilu, potem pozostaje on na Twoim urządzeniu | Gotowe profile ICC zawodzą. Zamiast tego dostarcz własny profil `.icc` |
| Radio internetowe | Nic osobistego - zapytanie o playlistę i strumień audio, kierowane do stacji (`api.somafm.com` oraz nazwany przez nią serwer icecast, `*.somafm.com`) | Tylko podczas odtwarzania opcjonalnego wbudowanego radia w odtwarzaczu dźwięku | Radio zawodzi. Każda inna funkcja dźwiękowa nadal działa |
| Adres URL, który każesz narzędziu przechwycić | Zapytanie do dokładnie tego adresu internetowego, który wpiszesz, wysyłane przez narzędzie zrzutu ekranu URL. Bez względu na to, jaki to adres. Ten host nie znajduje się w poniższej polityce, ponieważ wybierasz go w chwili użycia | Tylko po wpisaniu adresu URL w tym narzędziu i rozpoczęciu przechwytywania | Operator nie może dodać go do listy dozwolonych według hosta. Aby go usunąć, trzeba usunąć narzędzie |
| Weryfikacja podpisu SEAL | **Nic.** Aplikacja webowa w ogóle nie ma resolvera DNS - patrz niżej | Nigdy | Nie ma czego blokować |
| Modele detektora głębokiego skanowania | Nic osobistego - jednorazowe pobranie modelu z tego samego serwera (nie od strony trzeciej) | Tylko po włączeniu głębokiego skanowania w funkcji Verify | Głębokie skanowanie jest niedostępne. Standardowa weryfikacja nadal działa |
| Zdalna instancja | To, co odeśle nazwana przez Ciebie instancja, w ramach tej samej synchronizacji katalogu opisanej powyżej - plus znacznik wersji dołączany do zapytań do niej (rodzaj powłoki i wersja silnika, ta sama informacja, którą niesie user agent), dzięki czemu jej operator widzi, jakie wersje Lolly są w użyciu. W instancji zarządzanej, gdy jesteś zalogowany, ten znacznik niesie też identyfikator instalacji dla danego urządzenia, dzięki czemu lista urządzeń operatora może odróżnić tę instalację. Jest on dołączany wyłącznie do zapytań, które i tak wykonuje Twoje własne użytkowanie - nie ma tu żadnego licznika czasu i nic nie "dzwoni do domu" samoistnie - a opuszczenie instancji usuwa ten identyfikator, więc urządzenie, które połączy się ponownie później, przedstawia nowy. Wybierasz hosta w chwili użycia, więc nie znajduje się on w poniższej polityce | Tylko jeśli jawnie wskażesz powłoce inne wdrożenie Lolly | Przełączanie instancji zawodzi. Twoja lokalna instancja pozostaje nienaruszona |

Każdy stały host z tej tabeli jest jednocześnie pełną listą dozwolonych w polityce
Content-Security-Policy aplikacji, którą egzekwuje przeglądarka. Lista ta nie jest
więc jedynie opisem tego, co kod robi dzisiaj - to granica, do której przeglądarka
zobowiązuje aplikację: przyszła zmiana, która próbowałaby skontaktować się z jakimś
innym hostem, zostałaby zablokowana, a nie cicho dopuszczona. Dwa wiersze nie mają
stałego hosta, ponieważ adres wybierasz w chwili użycia: adres URL, o którego
przechwycenie prosisz narzędzie, oraz zdalna instancja, którą wskazujesz powłoce.
Żaden z nich nie znajduje się w polityce i każdy zachodzi tylko wtedy, gdy wpiszesz
adres i podejmiesz działanie. Wdrożenie, które nie chce żadnej z opcjonalnych
możliwości (na przykład instancja firmowa z własnymi czcionkami), usuwa te hosty ze
swojej polityki, a funkcje zawodzą w sposób bezpieczny, zamiast łączyć się z siecią.
