...
Narzędzia wektorowe (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, warianty logotypu, Diagram Builder) mogą eksportować do **DXF** - formatu wymiany AutoCAD R12, który odczytują wycinarki laserowe, plotery winylowe i oprogramowanie CNC/CAD. Geometria jest zapisywana jako **ścieżki konturowe w milimetrach** (krzywe spłaszczone z dużą dokładnością), tekst jest zamieniany na ścieżki konturowe, a kolor trafia jako najbliższy AutoCAD Color Index (który zwykle steruje narzędziem/operacją na wycinarce). DXF to wyłącznie grafika liniowa - obszar fotograficzny lub z filtrem nie ma formy ścieżki do cięcia i jest pomijany (Lolly ostrzega), więc do zachowania treści rastrowej użyj SVG/PDF.
...
**Myślenie.** Matematyka koloru działa percepcyjnie, a nie na surowych kanałach. Wyprowadzanie palety, rampy, harmonie i kontrast działają w **OKLCH/OKLab**, a kolor poza gamutem jest sprowadzany do zakresu własnym algorytmem mapowania gamutu z CSS Color 4 - redukcją nasycenia z kontrolą odległości percepcyjnej - zamiast przez przycinanie kanałów, więc żywy kolor trafia w najbliższy kolor, który faktycznie byś zaakceptował, a nie w spłaszczony. Gradienty interpolują w wybranej przestrzeni (domyślnie OKLab, lub `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, z kierunkiem przejścia odcienia dla przestrzeni biegunowych), a mieszanie jest **domnożone (premultiplied)**, więc przejście do przezroczystości zachowuje właściwy kolor zamiast ciemnieć po drodze w stronę czerni. Jeden interpolator obsługuje zarówno podgląd, jak i walkery eksportu - i to właśnie powstrzymało gradient stożkowy przed innym mieszaniem na ekranie, a innym w wyeksportowanym pliku.
