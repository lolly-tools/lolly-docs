...
Vektorové nástroje (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, logo lockupy, Diagram Builder) dokážou exportovat **DXF** - výměnný formát AutoCAD R12, který čtou laserové řezačky, vinylové plotry a software CNC/CAD. Geometrie se zapisuje jako obrysové **cesty v milimetrech** (křivky zploštěné na jemnou toleranci), text se převádí na cesty a barva dopadá na nejbližší index AutoCAD Color Index (ten na řezačce typicky řídí nástroj/operaci). DXF je čistě liniová grafika - fotografická nebo filtrovaná oblast nemá formu řezací cesty a je vynechána (Lolly na to upozorní), takže když potřebuješ zachovat rastrový obsah, použij SVG/PDF.
...
**Přemýšlení.** Barevná matematika probíhá perceptuálně, ne v surových kanálech. Odvozování palet, přechody, harmonie a kontrast běží v **OKLCH/OKLab** a barvu mimo gamut vrátí do rozsahu vlastní algoritmus mapování gamutu z CSS Color 4 - redukce sytosti s kontrolou perceptuální vzdálenosti - místo prostého ořezání kanálů, takže sytá barva skončí na nejbližší barvě, kterou bys skutečně přijal, ne na zploštělé. Gradienty se interpolují v prostoru, který si zvolíš (výchozí OKLab, nebo `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, se směrem putování odstínu u těch polárních) a míchání je **předvynásobené**, takže přechod do průhlednosti zůstane správně barevný místo toho, aby cestou tmavl k černé. Jeden interpolátor obsluhuje náhled i exportní walkery - a přesně to zastavilo situaci, kdy se kónický gradient míchal jinak na obrazovce a jinak v exportovaném souboru.
