...
이것이 유일한 진입로이며, 그래서 아무것도 보지 않은 채 브랜드에 들어오는 일이 없습니다. 스테이지 아래에는 두 개의 관리 패널이 있습니다:
...
레일 하단의 **버전**은 디자인 시스템이 더 이상 움직이는 표적이 아니게 되는 지점이에요. 하나를 게시하면 이 기기에 보관되는 **영구적이고 이름이 붙은 사본**을 얻게 돼요. 이후로는 절대 바뀌지 않으니, 이를 고정해 둔 도구는 계속 같은 것을 그리게 돼요. 패널은 게시할 나만의 무언가가 생기기 전까지는 숨겨져 있어서, 한 번도 게시하지 않은 스튜디오에는 이 장치가 보이지 않아요.

무언가를 누르기 전에 알아야 할 세 가지가 있고, 패널은 누른 뒤가 아니라 누르기 전에 세 가지를 모두 알려줘요.

- **버전은 영구적이에요.** 아직 삭제 기능이 없어서, 패널은 무엇이 보관되었고 계속 보관된다는 사실을 알려줄 뿐, 거짓말을 하는 버튼을 내놓지는 않아요.
- **삭제가 호환성 카드에서 가장 먼저 나와요.** 추가되거나 변경된 토큰은 소식이지만, *삭제된* 토큰은 도구를 망가뜨리는 원인이므로 가장 먼저 이름이 붙고 있는 그대로 불려요.
- **게시는 되돌릴 수 없지만, 복원은 되돌릴 수 있어요.** *이 버전에서 최신본으로 복원*은 헤드에 대한 일반적인 편집이라서 스튜디오의 실행 취소 스택에 올라가고, 패널은 바로 **실행 취소**를 제공해요.
