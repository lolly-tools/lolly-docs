...
- **템플릿에 EJS/임의 JS 없음.** XSS 대상 표면이 0입니다. 로직은 `hooks.js`에 있습니다.
- **필수 자산 CMS 없음.** 개인은 자신의 크리에이티브 파일을 앱 안에서 곧바로 자신의 카탈로그로 가져옵니다([카탈로그](/info/using.html) 뷰와 Brand Studio) - 서버도, 관리자 콘솔도 없습니다. 작업은 **세션**으로 전달됩니다: 공유 링크가 전체 상태를 담으며, 같은 세션이 백업이나 협업 세션을 통해서도 이동합니다. 배포를 관리하는 사람은 공유된 세션을 **템플릿**으로 고정할 수 있습니다 - 링크를 열어 그 값을 브랜드 팩 안 해당 도구 디렉터리의 템플릿 항목으로 기록하고 커밋하면, 이후 그 도구의 "New from template" 선택기에 나타나며 `?template=<id>`로 딥링크할 수 있습니다. Git은 배포 소유자의 고정 단계이며, 결코 제작자의 것이 아닙니다. *공유되고 통제된* 카탈로그를 원한다면, 조직은 같은 방식으로 자산 디렉터리를 관리하고 PR 검토를 통해 업데이트를 통제할 수 있습니다 - 이는 앱의 요구 사항이 아니라 선택 가능한 거버넌스 모델입니다.
- **강제 RBAC 없음.** 오픈 앱은 기본적으로 공개 접근이며, 브랜드 리스크는 성숙도 태그와 워터마크로 관리됩니다. 더 강한 통제를 원하는 조직은 자체 인증과 위의 git 검토 카탈로그를 그 위에 얹으면 됩니다.
- **중앙 데이터베이스 없음.** 모든 사용자 상태는 기기별로 저장됩니다. SUSE ID 통합은 로드맵에 있지만 출시를 가로막는 조건은 아닙니다.
- **공유되는 도구/엔진 코드 경로 없음.** 엔진은 오픈소스이며, `tools/`와 `assets/`는 각자의 저장소에서 SUSE 독점 콘텐츠로 남습니다. 이 분리는 강제됩니다(교차 임포트 없음). 그래서 분리 상태가 깔끔하게 유지됩니다.
...
`engine/`, `shells/`, `schemas/`, `docs/` 디렉터리는 **MPL-2.0** 하에 오픈소스입니다 - 브랜드 툴링을 위한 벤더 중립적 스캐폴딩 플랫폼으로, 배포 가능한 각 단위는 [github.com/lolly-tools](https://github.com/lolly-tools) 아래 자체 저장소로 분리되어 있습니다. `tools/`와 `catalog/assets/`는 SUSE 고유 콘텐츠로 **SUSE 독점**으로 남습니다(모든 권리 보유 - 각 저장소의 `NOTICE.md` 참고). 이들은 MPL의 적용을 받지 않습니다.

이 분리는 강제됩니다 - `engine/`에서 `tools/`나 `assets/`로의 교차 임포트가 없습니다 - 그래서 플랫폼/콘텐츠 경계가 깔끔하게 유지됩니다.
