...
### Strategi keselamatan & risikan berlapis pertahanan (defence-in-depth) - yang kebetulan pula menjadi platform pengeluaran kreatif

Sistem imun organisasi zero-trust yang membalut apa yang anda sudah lakukan - supaya kerja kreatif rutin yang diperlukan oleh pasukan anda setiap hari berlaku *di dalam* perimeter anda, bukannya bocor keluar daripadanya.

**Apa faedahnya untuk anda.** Anda menjadi orang yang berkata ya kepada sesuatu yang selamat *dan* popular. Anda menutup lubang eksfiltrasi, memperoleh keupayaan, dan memadamkan baris gilir permintaan dalam satu langkah - kemenangan keselamatan yang jarang berlaku yang menjadikan anda lebih disukai, bukan kurang. Tiada lagi panggilan jam 3 pagi daripada bahagian undang-undang kerana fail sulit (embargoed) atau data pelanggan tersesat masuk ke dalam alat web rawak; lebih sedikit vendor SaaS, kontrak, dan audit di atas meja anda; serta jejak audit yang sepenuhnya boleh dihasilkan semula yang boleh anda tunjukkan apabila seseorang bertanya. Anda tidur dengan lebih lena, dan mencerahkan beberapa hari sambil melakukannya.

Lolly bukan alat kreatif kelas kedua: ia meletakkan output bertaraf pengeluaran di tangan setiap orang, dan pengalaman penciptaan berpandukan jenama (brand-guided) yang ditawarkannya tiada tandingan. Sebab ia *selamat* untuk diagihkan seluas itu adalah bersifat seni bina: tiada apa yang dimuat naik melainkan apa yang anda sendiri letakkan di sana, setiap hasil boleh dihasilkan semula, dan setiap eksport boleh membawa berbilang lapisan rekod kriptografi bertaraf terkemuka dalam industri. Tidak kira bagaimana sesuatu dokumen sampai ke meja anda, anda boleh melihat asal-usulnya (provenance) secara penuh, sama ada ia telah diganggu, dan sama ada anda boleh menghasilkannya semula secara pixel-perfect.
...
- **Mampatkan PDF**
Kecilkan saiz PDF yang terlalu besar terus pada peranti (on-device), supaya tiada siapa perlu mencari laman web pihak ketiga untuk "mampatkan PDF saya" sebaik sahaja saiz fail terlalu besar untuk dihantar melalui e-mel - iaitu justeru tempat data paling mudah tergelincir keluar. 
...
- **Lolly Imprint.** Isyarat kedua yang saling melengkapi dan **aktif secara lalai**: tera air (watermark) piksel yang tidak kelihatan, dibenamkan ke dalam eksport raster (termasuk raster yang dirender oleh Lolly di dalam PDF/PPTX, tetapi tidak sekali-kali imej terbenam milik pengguna sendiri). Jika kelayakan (credential) itu terputus akibat sebarang perubahan pada bekas (container) fail, Lolly Imprint pula bertahan selepas fail disimpan semula atau ditangkap skrin - satu petunjuk tahan lasak bahawa "piksel ini pernah melalui Lolly", sekadar penanda kehadiran sahaja, tiada data peribadi terlibat. Ia bersifat keselamatan-melalui-kesamaran (security-through-obscurity), bukan pertahanan yang diperkukuh, dan ia melengkapi kelayakan tersebut, bukan menggantikannya. `imprint=0` untuk menolak keluar (opt out).
- **Durable Content Credentials (pilih masuk / opt-in).** Eksport raster boleh membawa tambahan tanda *durable* yang tidak kelihatan, yang mengekod pengecam soft-binding, supaya kelayakan C2PA masih boleh dipulihkan walaupun selepas muat naik ke media sosial atau simpan semula telah melucutkan metadata fail - iaitu keadaan di mana kelayakan biasa akan hilang. Ia khusus untuk raster sahaja dan memerlukan satu pas pengekodan neural (neural-encode), jadi ia tidak aktif secara lalai (`durable=1` untuk mengaktifkannya). Lolly mengenali tanda durable-nya sendiri secara luar talian pada `/verify` hari ini; pemulihan oleh alat pihak ketiga (contohnya Adobe) akan menyusul sebaik sahaja resolusi soft-binding industri itu tersedia.
- **Pengesahan berlaku pada peranti.** Lepaskan (drop) mana-mana fail pada `/verify` (atau `lolly validate <file>`) untuk mendapatkan laporan luar talian sama ada ia benar-benar dibuat dengan Lolly dan tidak berubah sejak itu. Paparan Verify pada web turut menandakan kandungan janaan AI, mengesan Lolly Imprint, mengesahkan tandatangan **SEAL** (tandatangan peringkat bait yang dikunci dalam DNS - satu-satunya sentuhan rangkaian ialah carian kunci DNS, tidak sekali-kali fail itu sendiri), secara pilihan menjalankan imbasan mendalam (deep-scan) untuk tera air piksel pihak ketiga (muat turun model sekali sahaja pada peranti), dan mendedahkan data tersembunyi - semuanya tanpa memuat naik fail tersebut. Lihat [Identiti Content Credentials](/info/content-credentials-identity.html).

> **Nota kesalinghubungan (interoperability).** Lolly mengesahkan kelayakannya sendiri dan banyak kelayakan pihak ketiga secara luar talian hari ini, termasuk membaca manifes tuntutan (claim) **v2** C2PA daripada penerbit lain. Satu perkara interop masih dalam proses: WebM - yang belum mempunyai pemetaan C2PA piawai, jadi Lolly melampirkan manifes tersebut sebagai bahagian Matroska (alat pihak ketiga mengesahkan MP4 Lolly secara terus; WebM akan menyusul sebaik sahaja piawai itu mantap).
...
Air-gap ialah **penggunaan kelas pertama (first-class deployment)**, bukan mod khas - Lolly berjalan tanpa rangkaian semasa render, sedia terus dari kotak (out of the box). Shell web adalah PWA offline-first (service worker); fon dan WASM disimpan pada peranti; keadaan (state) alat dikekalkan secara tempatan melalui host bridge, tidak sekali-kali `localStorage`. Mana-mana alat yang mencapai rangkaian hanya berbuat demikian melalui keupayaan `host.net` yang **disenaraikan-benar (allowlisted)** yang mesti diisytiharkan dalam manifesnya - shell yang tidak boleh (atau tidak mahu) memenuhinya akan menggantikannya dengan stub. Hantar shell ke peranti melalui MDM anda, atau sajikan satu instance di dalam rangkaian anda, dan pemasangan air-gapped sepenuhnya akan me-render, mengeksport, menyulitkan, serta mengesahkan kelayakan tanpa apa-apa untuk "phone home".
