...
descrizione di ciò che il codice fa oggi, è il confine che il browser impone all'
app: una futura modifica che tentasse di contattare un altro host verrebbe bloccata,
non permessa in silenzio. Due righe non hanno un host fisso, perché scegli tu
l'indirizzo al momento dell'uso: un URL che chiedi a uno strumento di catturare, e un'istanza
remota verso cui punti lo shell. Nessuna delle due è nell'informativa, ed entrambe avvengono solo
quando digiti un indirizzo e agisci su di esso. Un'installazione che non vuole nessuna
di quelle opzionali (un'istanza enterprise con i propri font, ad esempio) rimuove quegli
host dalla propria policy e le funzionalità falliscono in modo chiuso invece di connettersi.
