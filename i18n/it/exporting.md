...
Gli strumenti vettoriali (Brand Lockup, QR Code, Street Map, Wayfinding Signage, Pose Geeko, i lockup del logo, Diagram Builder) possono esportare in **DXF** - il formato di interscambio AutoCAD R12 che tagliatrici laser, plotter vinilici e software CNC/CAD sanno leggere. La geometria viene scritta come percorsi di contorno **in millimetri** (le curve vengono appiattite con una tolleranza fine), il testo viene trasformato in contorni e il colore viene mappato sull'AutoCAD Color Index più vicino (che tipicamente pilota lo strumento/l'operazione su una tagliatrice). Il DXF è solo line-art - una regione fotografica o filtrata non ha una forma esprimibile come percorso di taglio e viene scartata (Lolly avverte), quindi usa SVG/PDF quando devi conservare contenuto raster.
...
**Ragionamento.** La matematica del colore avviene in modo percettivo anziché sui canali grezzi. Derivazione della palette, rampe, armonie e contrasto girano in **OKLCH/OKLab**, e un colore fuori gamut viene riportato nel range dall'algoritmo di gamut-mapping proprio di CSS Color 4 - riduzione della crominanza con un controllo di distanza percettiva - anziché tagliando i canali, così un colore vivido finisce sul colore più vicino che accetteresti davvero, invece che su uno appiattito. I gradienti interpolano in uno spazio a tua scelta (OKLab di default, oppure `oklch`, `lab`, `lch`, `srgb`, `srgb-linear`, `hsl`, con una direzione di percorso della tonalità per quelli polari), e la miscelazione è **premoltiplicata**, così una dissolvenza verso la trasparenza mantiene il colore giusto invece di scurirsi verso il nero lungo il percorso. Un unico interpolatore serve sia l'anteprima che i walker di esportazione - ed è ciò che ha impedito a un gradiente conico di essere miscelato in un modo a schermo e in un altro nel file esportato.
