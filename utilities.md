...
## Take a PDF apart - `#/pdf`

A PDF is a container, and most software treats it as one opaque thing. This view opens it: the words, the vector marks, the images, the fonts and the attachments, each viewable and downloadable. Drop a `.pdf` or an `.ai`. Nothing is uploaded.

The **Text** pass rebuilds each page in reading order with the page's own vector picture beside it, noting how many columns it was read as and how many rotated runs were left out, so you can judge the reconstruction. Take it away with **Copy all**, **Download .md** or **Download .txt**, or copy one page at a time.
...
The other tabs appear only when a pass found something, so the strip describes your document rather than what a PDF could theoretically contain: **Logos** (vector marks, downloadable as SVG and usually the most useful thing in a guidelines PDF, since they stay sharp at any size), **Images** (rasters, with undecodable ones counted rather than hidden), **Fonts** (each with its own embedding caveats stated plainly - a subset is called a subset, and "no embedding restriction" is not called a licence) and **Attachments**.
