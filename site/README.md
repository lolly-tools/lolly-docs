...
| `covers.json` | 2 - covers | No heading row above the fan by design. `covers[]`: `img` is a path under `/info/shots/` (a posed 4:3 still in `docs/shots/covers/`), `href` an app route, `name`/`promise`/`alt` copy, `hue` the OKLCH hue the cover was posed in. Add `video` (a WebM under `/info/shots/`, e.g. `covers/3d.webm`) to make a cover a ~6 s loop with `img` as its poster - Backdrop, Audiogram and 3D ship that way. The covers are POSED, not hand-made: `node scripts/build-covers.ts` (against a running `dev:web` on the lolly-start profile) captures each one as the app wearing a derived design system a hue apart (16 covers, 22.5° steps), so the fan reads as a rainbow, and the fan opens on the cover nearest the reader's own accent (`lib/covers-flow.ts`). Keep `hue` and the order in step with the script's `POSES`. |
