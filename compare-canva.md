...
Every asset comes from a tool you can inspect and reuse, with Content Credentials sealed into the file (a credential signed on the device reads as unverified in a stranger's validator until [an identity anchors it](/info/content-credentials-identity.html)). Approve the tool once and every output is on-brand by construction, at any volume. The line that holds is no longer the subscription: it is that Lolly needs no account and makes no connection you did not ask for. Canva's offline render works without Wi-Fi; activating the product still requires a verified account, and the apps make connections that cannot be declined from inside them. Lolly opens from a plain URL, and the only traffic is what you asked for.
