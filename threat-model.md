...
| **Tool hook → the user's signing identity** (`host.c2pa.sign`) | A tool hook calling `host.c2pa?.sign(bytes, format, opts)`, the optional v1.85 bridge API declared at `packages/core/src/host-v1.ts:375`–`:379` (interface at `:2131`–`:2158`). Two shipping tools use it: `community/redact` and `community/embed-track-image` | There is no `capabilities` gate on it - the API is optional and additive, so a tool feature-detects rather than declares it. The web shell's facade (`shells/web/src/bridge/index.ts:467`) lazily routes to `signFreshC2pa` (`shells/web/src/bridge/export.ts:5364`), which signs through `signAndEmbedC2pa` (`:5260`): that asks `host.identity.signer()` first (`:5274`), so an **enrolled** user's certificate chain goes into the manifest, and falls back to the engine's ephemeral self-signed key when no valid certificate is cached. Signing is local; nothing is uploaded | `tests/redact.test.ts:636` pins the call shape and asserts the tool ships what `sign` returned; `:663` pins the no-identity path | A hook can put the user's CA-verified identity on bytes of its own choosing, and the manifest it produces is a real one - this is the ordinary export path's `c2pa` gate deliberately not applying, because the caller is the file's author. Tool review is the control, exactly as for `{{{x}}}`. The blast radius is bounded by certificate lifetime (7–365 days) and by the fact that the hook can only sign in a session the user already enrolled. |
...
|---|---|---|---|
| Tool hooks are not sandboxed **on the default path**. `new Function('host', …)` injects the bridge into closure scope but runs in the realm's globals, so a hook in a browser shell can reach `window`, `document` and `fetch`. | `engine/src/runtime.ts:1030`–`:1038`; `engine/src/runtime.ts:14`–`:18`; `CLAUDE.md`, "The runtime lifecycle" | Some shipping tools legitimately rely on realm access, and every tool in the catalogue is first-party reviewed content served from the deployment's own origin. Confinement would break tools without changing who can publish them. | Worker isolation has landed as a **per-tool opt-in**: a manifest declaring `isolate: true` (`schemas/tool.schema.json:386`) routes its hooks through a Worker-backed executor in the live tool view (`shells/web/src/bridge/hook-worker.ts`, `shells/web/src/lib/mount-runtime.ts:39`–`:81`), and a shell with no such executor silently runs in-realm, so the flag is safe to declare anywhere. It is opt-in and set only once a tool's hooks are verified never to touch DOM globals - `community/chart-creator` is the first - so the default path, and every un-opted tool, is still in-realm. The `Content-Security-Policy` remains the containment underneath both: its `connect-src` allowlist stops an executing hook posting data to a host outside the allowlist (`deploy/docker/nginx.conf:31`, `tests/security-headers.test.ts`). Running third-party or untrusted tool code stays out of scope, and the catalogue-signing path (`engine/src/catalog-integrity.ts`) is the control a deployment can turn on today. |
...
CI runs nine jobs (`.github/workflows/ci.yml`): `test`, `typecheck`,
...
(a self-test of the composite render action), `audit` (`npm audit`, high and
critical) and `sast` (Opengrep: Lolly's own rules full-tree and blocking,
`--no-git-ignore` so submodule files are actually scanned; the upstream packs
run diff-only and advisory). Lint is not among them, by design.
