...
- <!--i:globe--> **It publishes anyway.** It can. What it cannot do is produce a file that answers "did SUSE make this?" with yes. Anyone can drop the file into [Verify](https://lolly.tools/verify) and get an anonymous signer, a broken credential or no credential. Meanwhile a genuine SUSE export answers with a CA-verified identity. The forgery does not need to be blocked. It needs to be distinguishable, and it is.
:::
