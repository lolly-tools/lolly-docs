...
- `hooks` - `{ onInit?, onInput?, onFrame?, onLevel?, beforeExport?, afterExport?, exportFile? }` boolean flags. If any are true, you must ship `hooks.js` with the matching functions. (`exportFile` is the transform path - file bytes in → transformed bytes out; `onFrame` makes the tool react to a live camera; `onLevel` makes it react to live audio levels while recording - all covered below.) The schema also accepts `beforeRender`, but the runtime has no invocation site for it yet, so declaring it does nothing.
