...
- **Checking a file.** Lolly verifies its own credentials too: drop any file on [/valid](/valid) (or run `lolly validate <file>` in the CLI) for an on-device report — headlined by whether the file was genuinely made with Lolly and unchanged since.
