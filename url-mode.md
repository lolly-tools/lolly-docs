...
| `loop` | web only (present mode) | Presence flag - the presenter **wraps** at the ends and a timed document's transport loops, so `?present&loop` is digital signage (pair with per-frame `dur` for dwell). **Not a reserved param**: `loop` is a real input id in several tools (Slides, Deck Builder, 3D, …), so it is read as a kiosk flag only in the Design tool, which has no `loop` input. |
