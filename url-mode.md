...
- **Channels:** `x12.5`, `y-40`, `s1.2`, `r15`, `z140`, `o0.8`, `b2.5`, the size pair `w1280` / `h720`, the tilt pair `rx-8` / `ry20`, plus camera-only `f`/`a`/`p`. Tilt is parsed from day one and **ignored by every consumer until the tilt export tier ships** - a track that carries it renders today exactly as if it did not.
