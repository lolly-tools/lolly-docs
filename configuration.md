...
- **Engine version** — `ENGINE_VERSION` in `engine/src/index.ts` (currently `1.29.0`); the capability bridge is additive-only within a major, so shells and tools built against an older minor keep working. See [Host API](/info/host-api.html).
