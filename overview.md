...
|---|---|---|
| `everyone` | QR Code Generator, Quote Card, Email Signature, Day Brief, Code Canvas, Color Block, Dynamic Layout, Logo, Wordmark, Web Icon Maker, Mesh Gradient, Diagram Builder, Flow Chart, Asset Export, Audiogram, Sequence Studio, Voice Recorder, Record, Top & Tail Video | Employee Image Stationery |
| `designer` | Brand Lockup, Layout Studio, Bag Video, Chart Creator, D3 Chart Studio, Street Map, Animated Ad, Lottie Ad, Multi-Page PDF, Doc Studio, Carousel Maker, Deck Builder, Deck Studio, Slides, 3D, Pose Geeko, Logo Lockup: Grid (NASCAR), Logo Lockup: Partner, Filter: Duotone, Filter: Halftone, Filter: Scanline, Filter: Posterize Bitmap, Filter: Pixel Stretch, Filter: Voronoi Cells | Font Outliner |
| `event` | Meeting Planner, Event Name Badge, Wayfinding Signage, Calendar ICS, Booth Studio | Event Stationery, Bulk Name Badges, Room Agenda Cards |
| `product` | - | CVE Alert, Product Release Announcement, Blog OG Image |
| `utility` | Countdown Timer, Color Picker, Palette Lab, URL Screenshot, Screen Capture, Strip Hidden Data, Text Helper, Compress PDF, Convert Image, Prompt to Image, Rebrand a Deck, Embed & Track Image | Unit/format converters, more on-device privacy utilities |

That's 60 distinct tools across the two packs mounted by the active profile - the brand-agnostic `community/` set plus the active brand's own. (Layout Studio and Voice Recorder exist in more than one pack under the same tool id; only the active profile's copy mounts, so they count once.) Counting from the manifests is the only reliable way - `ls community/*/tool.json brands/*/tools/*/tool.json` - since the table above is maintained by hand and the packs move independently.

Tools are also classified by status: `official` (brand approved, no watermark), `community` (external contribution), `experimental` (watermarked exports). Currently `experimental`: 3D, Booth Studio, Diagram Builder, Dynamic Layout, Filter: Posterize Bitmap, Filter: Voronoi Cells, Logo Lockup: Grid (NASCAR), Record, Top & Tail Video, URL Screenshot and Voice Recorder. Shipping as `community`: Audiogram, Carousel Maker, Convert Image, Deck Builder, Deck Studio, Flow Chart, Layout Studio, Lottie Ad, Palette Lab, Screen Capture, Sequence Studio, Slides, Web Icon Maker and Wordmark. Everything else is `official`.
...
| `host.pdf` | 1.8 | PDF analyse / strip / compress - Strip Hidden Data, Compress PDF |
...
| `host.pptx` | 1.58 | Inspect and re-theme an uploaded `.pptx` (the Rebrand a Deck path) |
| `host.images` | 1.60 | Decode / resize / re-encode bytes on device. **Contract only so far** - declared in the SDK, not yet implemented by a shell |
...
`url-mode.ts` handles the round-trip (parse and serialize). Reserved params (never forwarded to the tool as inputs) - the `RESERVED` set in `engine/src/url-mode.ts` is the authority, and [URL Mode](/info/url-mode.html) documents each one: `format`, `export`, `copy`, `slot`, `output`, `filename`, `_v`, `width`/`w`, `height`/`h`, `unit`, `dpi`, `profile`, `password`, `bleed`, `marks`, `c2pa`, `imprint`, `durable`, `hdr`, `cuts`, `lang`, `full`, `options`, `nostage`, `z` (packed state - the "Shortest link" token) and `zx` (the same, encrypted under a password). Asset inputs in URL mode are serialised by their `id`; the runtime resolves them via `host.assets.get()` before hydration. `width`/`height` are values in `unit` (default `px`, also `mm`/`cm`/`in`/`pt`/`pc`); with a physical unit `dpi` sets raster resolution. They set the canvas document size and pre-fill the export dimensions panel.
...
![Nine steps across four hues, all grown from the single seed colour carried in the link](/t/url-shot?url=%2F%23%2Ftool%2Fcolor-palette%3Fseed%3De0521a%26harmony%3Dtetrad-4%26steps%3D9%26full&width=1440&height=900&dpi=192&waitMs=2000&format=svg&cropSelector=%23tool-canvas&walker=1&filename=ov2-url-palette)
...
![The opening slide of the default deck, whose own subtitle states that every slide can hold another Lolly tool](/t/url-shot?url=%2F%23%2Ftool%2Fslides%3FfocusSlide%3D1%26full&width=1440&height=900&dpi=192&waitMs=2600&format=svg&cropSelector=%23tool-canvas&walker=1&filename=ov2-slides-deck)
...
- **No mandatory asset CMS.** Individuals ingest their own creative files straight into their catalogue in-app (the [Catalogue](/info/using.html) view and the Brand Studio) and author their own tools by saving [Layout Studio](/info/using.html) sessions - no server, no admin console. For a *shared, governed* catalog, an organisation **can** manage the asset directory as git and gate updates through PR review - that's an available governance model, not a requirement of the app.
